...
static void DebugAction_Party_AllAbilityBattlePattern(u8 taskId, enum DebugTrainerIds playerTrainerId, enum DebugTrainerIds aiTrainerId)
{
    ZeroPlayerPartyMons();
    ZeroEnemyPartyMons();
    CreateNPCTrainerPartyFromTrainer(gParties[B_TRAINER_PLAYER], &sDebugTrainers[DIFFICULTY_NORMAL][playerTrainerId], TRUE, BATTLE_TYPE_TRAINER);
    CreateNPCTrainerPartyFromTrainer(gParties[B_TRAINER_OPPONENT_A], &sDebugTrainers[DIFFICULTY_NORMAL][aiTrainerId], FALSE, BATTLE_TYPE_TRAINER);
...
static void DebugAction_Party_StartTeamViewerBattle(u8 taskId, u16 trainerId)
{
    ...
    {
        CreateMonWithIVs(&gParties[B_TRAINER_PLAYER][i], sPlayerSpecies[i], 100, i + 1, OTID_STRUCT_PLAYER_ID, MAX_PER_STAT_IVS);
        for (j = 0; j < MAX_MON_MOVES; j++)
            SetMonMoveSlot(&gParties[B_TRAINER_PLAYER][i], sPlayerMoves[j], j);
    }
    HealPlayerParty();
