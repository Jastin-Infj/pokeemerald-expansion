...
static void ZeroTrainerPartyBuffer(struct Pokemon *party)
{
    u32 i;
...
static u8 CreateNPCTrainerPartyFromTrainerInternal(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags, u16 *opponentMonCanTera, u16 *opponentMonCanDynamax)
{
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
    ...
    retVal = CreateNPCTrainerPartyFromTrainerInternal(party, trainer, firstTrainer, battleTypeFlags, &opponentMonCanTera, &opponentMonCanDynamax);
...
u8 CreateNPCTrainerPartyForPreview(struct Pokemon *party, u16 trainerNum, u32 battleTypeFlags, u16 *opponentMonCanTera, u16 *opponentMonCanDynamax)
{
    ...
    if (GetTrainerStructFromId(trainerNum)->overrideTrainer)
    {
        ...
        retVal = CreateNPCTrainerPartyFromTrainerInternal(party, &tempTrainer, TRUE, battleTypeFlags, opponentMonCanTera, opponentMonCanDynamax);
    }
    else
    {
        retVal = CreateNPCTrainerPartyFromTrainerInternal(party, GetTrainerStructFromId(trainerNum), TRUE, battleTypeFlags, opponentMonCanTera, opponentMonCanDynamax);
    }
    return retVal;
}
...
#define HAS_SPEED_ABILITY(targetAbility) \
    ...
    if (HasWeatherEffect())
    {
        if (HAS_SPEED_ABILITY(ABILITY_SWIFT_SWIM) && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            speed *= 2;
        if (HAS_SPEED_ABILITY(ABILITY_CHLOROPHYLL) && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_SUN)
            speed *= 2;
        if (HAS_SPEED_ABILITY(ABILITY_SAND_RUSH) && gBattleWeather & B_WEATHER_SANDSTORM)
            speed *= 2;
        if (HAS_SPEED_ABILITY(ABILITY_SLUSH_RUSH) && (gBattleWeather & B_WEATHER_ICY_ANY))
            speed *= 2;
    }
    ...
    if (HAS_SPEED_ABILITY(ABILITY_SLOW_START) && gBattleMons[battler].volatiles.slowStartTimer != 0)
        speed /= 2;
    if (HAS_SPEED_ABILITY(ABILITY_PROTOSYNTHESIS) && !(gBattleMons[battler].volatiles.transformed) && ((gBattleWeather & B_WEATHER_SUN && HasWeatherEffect()) || gBattleMons[battler].volatiles.boosterEnergyActivated))
...
static void HandleEndTurn_FinishBattle(void)
{
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
        ...
        FadeOutMapMusic(5);
        if (B_TRAINERS_KNOCK_OFF_ITEMS == TRUE || B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9 || B_RESTORE_HELD_BATTLE_BERRIES == TRUE)
            TryRestoreHeldItems();
