...
static bool32 ShouldTeraShellDistortTypeMatchups(enum Move move, enum BattlerId battlerDef, enum Ability abilityDef)
{
    if (!gSpecialStatuses[battlerDef].distortedTypeMatchups
     ...
     && (abilityDef == ABILITY_TERA_SHELL || (gAllAbilitySlotsBattle && battlerDef < gBattlersCount && BattlerHasAbility(battlerDef, ABILITY_TERA_SHELL))))
        return TRUE;
...
void PrepareStringBattle(enum StringID stringId, enum BattlerId battler)
{
    ...
    case STRINGID_PKMNCUTSATTACKWITH:
        if (GetConfig(B_UPDATED_INTIMIDATE) >= GEN_8
         && BattlerHasAbility(gBattlerTarget, ABILITY_RATTLED)
        ...
        else if (BattlerHasAbility(gBattlerTarget, ABILITY_CONTRARY))
        {
...
bool32 CanTargetBlockPranksterMove(struct BattleContext *ctx, s32 movePriority)
{
    if (movePriority <= 0
     ...
     || (IsBattleMoveStatus(ctx->move) && (BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_MAGIC_BOUNCE) || gProtectStructs[ctx->battlerDef].bounceMove)))
        return FALSE;
...
static bool32 IsPowderMoveBlocked(struct BattleContext *ctx)
{
    enum Ability blockingAbility = ctx->abilityDef;
    ...
    if (ctx->runScript)
    {
        if (BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_OVERCOAT))
...
static bool32 CanAbilityAbsorbMoveWithAbility(struct BattleContext *ctx, enum Ability abilityDef)
{
    ...
    case ABILITY_MOTOR_DRIVE:
        if (ctx->moveType == TYPE_ELECTRIC)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, abilityDef, STAT_SPEED, 1);
        break;
    case ABILITY_LIGHTNING_ROD:
        if (GetConfig(B_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && ctx->moveType == TYPE_ELECTRIC)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, abilityDef, STAT_SPATK, 1);
        break;
    ...
    case ABILITY_SAP_SIPPER:
        if (ctx->moveType == TYPE_GRASS)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, abilityDef, STAT_ATK, 1);
        break;
    case ABILITY_WELL_BAKED_BODY:
        if (ctx->moveType == TYPE_FIRE)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, abilityDef, STAT_DEF, 2);
        break;
...
bool32 CanAbilityAbsorbMove(struct BattleContext *ctx)
{
    ...
    return CanAbilityAbsorbMoveWithAbility(ctx, ctx->abilityDef);
}
        ...
        case ABILITY_DOWNLOAD:
            if (shouldAbilityTrigger)
            {
                ...
                if (CompareStat(battler, statId, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility))
                {
                    ...
                    BattleScriptCallWithBattlerAbility(battler, gLastUsedAbility, BattleScript_AttackerAbilityStatRaiseEnd3);
                    effect++;
                }
            }
            break;
        ...
        case ABILITY_INTREPID_SWORD:
            if (shouldAbilityTrigger && !GetBattlerPartyState(battler)->intrepidSwordBoost)
            {
                ...
                if (CompareStat(battler, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility))
                {
                    ...
                    BattleScriptCallWithBattlerAbility(battler, gLastUsedAbility, BattleScript_BattlerAbilityStatRaiseOnSwitchIn);
                    effect++;
                }
            }
            break;
...
bool32 IsBattlerProtected(struct BattleContext *ctx)
{
    ...
    if (gProtectStructs[ctx->battlerDef].protected != PROTECT_MAX_GUARD && !MoveIgnoresProtect(ctx->move))
    {
        ...
        if (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_UNSEEN_FIST)
...
static inline u32 CalcMoveBasePower(struct BattleContext *ctx)
{
    ...
    case EFFECT_DOUBLE_POWER_ON_ARG_STATUS:
        ...
        if ((gBattleMons[battlerDef].status1 | (STATUS1_SLEEP * BattlerHasAbilityForCalc(battlerDef, ctx->abilityDef, ABILITY_COMATOSE))) & GetMoveEffectArg_Status(move)
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    if (gAllAbilitySlotsBattle)
    {
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_TECHNICIAN)
         && basePower <= 60)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_FLARE_BOOST)
         && gBattleMons[battlerAtk].status1 & STATUS1_BURN
         && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_TOXIC_BOOST)
         && gBattleMons[battlerAtk].status1 & STATUS1_PSN_ANY
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_RECKLESS)
         && (moveEffect == EFFECT_RECOIL || moveEffect == EFFECT_RECOIL_IF_MISS))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_IRON_FIST)
         && IsPunchingMove(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SHEER_FORCE)
         && MoveIsAffectedBySheerForce(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SAND_FORCE)
         && (moveType == TYPE_STEEL || moveType == TYPE_ROCK || moveType == TYPE_GROUND)
         && ctx->weather & B_WEATHER_SANDSTORM)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_RIVALRY))
        {
            if (AreBattlersOfSameGender(battlerAtk, battlerDef))
                modifier = uq4_12_multiply(modifier, UQ_4_12(1.25));
            else if (AreBattlersOfOppositeGender(battlerAtk, battlerDef))
        ...
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_ANALYTIC)
         && IsLastMonToMove(battlerAtk)
         && moveEffect != EFFECT_FUTURE_SIGHT)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_TOUGH_CLAWS)
         && IsMoveMakingContact(battlerAtk, battlerDef, ctx->abilityAtk, ctx->holdEffectAtk, ctx->move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_STRONG_JAW)
         && IsBitingMove(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_MEGA_LAUNCHER)
         && IsPulseMove(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_WATER_BUBBLE)
         && moveType == TYPE_WATER)
            modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_STEELWORKER)
         && moveType == TYPE_STEEL)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_PIXILATE)
         && moveType == TYPE_FAIRY
         && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(B_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_GALVANIZE)
         && moveType == TYPE_ELECTRIC
         && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(B_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_REFRIGERATE)
         && moveType == TYPE_ICE
         && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(B_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_AERILATE)
         && moveType == TYPE_FLYING
         && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(B_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_NORMALIZE)
         && moveType == TYPE_NORMAL
         && gBattleStruct->battlerState[battlerAtk].ateBoost
         && GetConfig(B_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_PUNK_ROCK)
         && IsSoundMove(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_STEELY_SPIRIT)
         && moveType == TYPE_STEEL)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SHARPNESS)
         && IsSlicingMove(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SUPREME_OVERLORD))
            modifier = uq4_12_multiply(modifier, GetSupremeOverlordModifier(battlerAtk));
    }
    ...
    if (gAllAbilitySlotsBattle)
    {
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_HEATPROOF)
         && moveType == TYPE_FIRE)
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_WATER_BUBBLE)
         && moveType == TYPE_FIRE)
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_DRY_SKIN)
         && moveType == TYPE_FIRE)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.25));
    }
...
static inline u32 CalcAttackStat(struct BattleContext *ctx)
{
    ...
    if (BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_UNAWARE))
        atkStage = DEFAULT_STAT_STAGE;
    ...
    if (gAllAbilitySlotsBattle)
    {
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_HUGE_POWER)
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_PURE_POWER)
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SLOW_START)
         && gBattleMons[battlerAtk].volatiles.slowStartTimer > 0
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SOLAR_POWER)
         && IsBattleMoveSpecial(move)
         && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_DEFEATIST)
         && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 2))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_FLASH_FIRE)
         && moveType == TYPE_FIRE
         && gBattleMons[battlerAtk].volatiles.flashFireBoosted)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_SWARM)
         && moveType == TYPE_BUG
         && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_TORRENT)
         && moveType == TYPE_WATER
         && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_BLAZE)
         && moveType == TYPE_FIRE
         && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_OVERGROW)
         && moveType == TYPE_GRASS
         && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (IsBattleMoveSpecial(move) && IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
        {
            ...
            bool32 attackerHasPlus = BattlerHasAbilityForCalc(battlerAtk, ctx->abilityAtk, ABILITY_PLUS);
            bool32 attackerHasMinus = BattlerHasAbilityForCalc(battlerAtk, ctx->abilityAtk, ABILITY_MINUS);
        ...
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_FLOWER_GIFT)
         && gBattleMons[battlerAtk].species == SPECIES_CHERRIM_SUNSHINE
         && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN)
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_HUSTLE)
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_GUTS)
         && gBattleMons[battlerAtk].status1 & STATUS1_ANY
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_STAKEOUT)
         && gBattleStruct->battlerState[battlerDef].isFirstTurn == 2)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_TRANSISTOR)
         && moveType == TYPE_ELECTRIC)
        {
            if (GetConfig(B_TRANSISTOR_BOOST) >= GEN_9)
                modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        ...
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_DRAGONS_MAW)
         && moveType == TYPE_DRAGON)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_GORILLA_TACTICS)
         && IsBattleMovePhysical(move)
         && !IsGimmickSelected(battlerAtk, GIMMICK_DYNAMAX)
         && GetActiveGimmick(battlerAtk) != GIMMICK_DYNAMAX)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_ROCKY_PAYLOAD)
         && moveType == TYPE_ROCK)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_PROTOSYNTHESIS)
        ...
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_QUARK_DRIVE)
         && !(gBattleMons[battlerAtk].volatiles.transformed))
        {
            enum Stat atkHighestStat = GetParadoxBoostedStatId(battlerAtk);
            if ((ctx->fieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gBattleMons[battlerAtk].volatiles.boosterEnergyActivated)
        ...
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_ORICHALCUM_PULSE)
         && ctx->weather & B_WEATHER_SUN
         && IsBattleMovePhysical(move)
         && ctx->holdEffectAtk != HOLD_EFFECT_UTILITY_UMBRELLA)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3333));
        if (BattlerHasAdditionalAbility(battlerAtk, ctx->abilityAtk, ABILITY_HADRON_ENGINE)
    ...
    if (gAllAbilitySlotsBattle)
    {
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_THICK_FAT)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_PURIFYING_SALT)
         && moveType == TYPE_GHOST)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[partner].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(partner, B_WEATHER_SUN) && IsBattleMovePhysical(move))
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        default:
            break;
        ...
        if (gAllAbilitySlotsBattle
         ...
         && IsBattlerWeatherAffected(partner, B_WEATHER_SUN)
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    }

    // Ruin field effects
    if (IsBattleMoveSpecial(move) && !BattlerHasAbilityForCalc(battlerAtk, ctx->abilityAtk, ABILITY_VESSEL_OF_RUIN) && IsRuinStatusActive(VOLATILE_VESSEL_OF_RUIN))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));

    if (IsBattleMovePhysical(move) && !BattlerHasAbilityForCalc(battlerAtk, ctx->abilityAtk, ABILITY_TABLETS_OF_RUIN) && IsRuinStatusActive(VOLATILE_TABLETS_OF_RUIN))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    if (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_UNAWARE))
        defStage = DEFAULT_STAT_STAGE;
    ...
    if (gAllAbilitySlotsBattle)
    {
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_MARVEL_SCALE)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_FUR_COAT)
         && usesDefStat)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_GRASS_PELT)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_FLOWER_GIFT)
         && gBattleMons[battlerDef].species == SPECIES_CHERRIM_SUNSHINE
         && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN)
         && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_PROTOSYNTHESIS)
         && !(gBattleMons[battlerDef].volatiles.transformed))
        {
            enum Stat defHighestStat = GetParadoxBoostedStatId(battlerDef);
            if (((ctx->weather & B_WEATHER_SUN && HasWeatherEffect()) || gBattleMons[battlerDef].volatiles.boosterEnergyActivated)
        ...
        if (BattlerHasAdditionalAbility(battlerDef, ctx->abilityDef, ABILITY_QUARK_DRIVE)
         && !(gBattleMons[battlerDef].volatiles.transformed))
        {
            u32 defHighestStat = GetParadoxBoostedStatId(battlerDef);
            if ((gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gBattleMons[battlerDef].volatiles.boosterEnergyActivated)
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerDef)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[partner].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(partner, B_WEATHER_SUN) && !usesDefStat)
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        default:
            break;
    ...
    // Ruin field effects
    if (usesDefStat && !BattlerHasAbilityForCalc(battlerDef, ctx->abilityDef, ABILITY_SWORD_OF_RUIN) && IsRuinStatusActive(VOLATILE_SWORD_OF_RUIN))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));

    if (!usesDefStat && !BattlerHasAbilityForCalc(battlerDef, ctx->abilityDef, ABILITY_BEADS_OF_RUIN) && IsRuinStatusActive(VOLATILE_BEADS_OF_RUIN))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));
...
{
    bool32 hasAdaptability = BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_ADAPTABILITY);
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(struct BattleContext *ctx)
{
    ...
    bool32 hasGuts = ctx->abilityAtk == ABILITY_GUTS;

    if (!hasGuts)
        hasGuts = BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_GUTS);
...
static inline uq4_12_t GetScreensModifier(struct BattleContext *ctx)
{
    ...
    if (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_INFILTRATOR) && !IsBattlerAlly(ctx->battlerAtk, ctx->battlerDef))
    {
        if (ctx->updateFlags)
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct BattleContext *ctx)
{
    ...
    if ((ctx->abilityDef == ABILITY_MULTISCALE || ctx->abilityDef == ABILITY_SHADOW_SHIELD)
    ...
    if ((ctx->abilityDef == ABILITY_FILTER || ctx->abilityDef == ABILITY_SOLID_ROCK || ctx->abilityDef == ABILITY_PRISM_ARMOR)
    ...
    if (ctx->abilityDef == ABILITY_FLUFFY)
    {
    ...
    if (ctx->abilityDef == ABILITY_PUNK_ROCK && IsSoundMove(ctx->move))
    {
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerDef, ABILITY_PUNK_ROCK);
    }
    if (ctx->abilityDef == ABILITY_ICE_SCALES && IsBattleMoveSpecial(ctx->move))
    {
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        if (ctx->updateFlags)
    ...
    if (gAllAbilitySlotsBattle)
    {
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_MULTISCALE)
         && IsBattlerAtMaxHp(ctx->battlerDef))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_SHADOW_SHIELD)
         && IsBattlerAtMaxHp(ctx->battlerDef))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_FILTER)
         && ctx->typeEffectivenessModifier >= UQ_4_12(2.0))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.75));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_SOLID_ROCK)
         && ctx->typeEffectivenessModifier >= UQ_4_12(2.0))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.75));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_PRISM_ARMOR)
         && ctx->typeEffectivenessModifier >= UQ_4_12(2.0))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.75));
            if (ctx->updateFlags)
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_FLUFFY))
        {
            if (ctx->moveType == TYPE_FIRE && !IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ctx->abilityAtk, ctx->holdEffectAtk, ctx->move))
            {
                modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_PUNK_ROCK)
        ...
        if (BattlerHasAdditionalAbility(ctx->battlerDef, ctx->abilityDef, ABILITY_ICE_SCALES)
         && IsBattleMoveSpecial(ctx->move))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
...
static inline uq4_12_t GetDefenderPartnerAbilitiesModifier(enum BattlerId battlerPartnerDef)
{
    ...
    if (BattlerHasAbilityForCalc(battlerPartnerDef, GetBattlerAbility(battlerPartnerDef), ABILITY_FRIEND_GUARD))
        return UQ_4_12(0.75);
...
static inline uq4_12_t GetDefenderItemsModifier(struct BattleContext *ctx)
{
    ...
    case HOLD_EFFECT_RESIST_BERRY:
        ...
        if (ctx->moveType == GetBattlerHoldEffectParam(ctx->battlerDef) && (ctx->moveType == TYPE_NORMAL || ctx->typeEffectivenessModifier >= UQ_4_12(2.0)))
        {
            ...
            return BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_RIPEN) ? UQ_4_12(0.25) : UQ_4_12(0.5);
        }
        break;
    default:
        break;
...
s32 CalcCritChanceStage(struct BattleContext *ctx)
{
    ...
    if (gSideStatuses[GetBattlerSide(ctx->battlerDef)] & SIDE_STATUS_LUCKY_CHANT)
    {
        critChance = CRITICAL_HIT_BLOCKED;
          ...
          || (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_MERCILESS) && gBattleMons[ctx->battlerDef].status1 & STATUS1_PSN_ANY))
    {
    ...
    else
    {
        critChance  = (gBattleMons[ctx->battlerAtk].volatiles.focusEnergy != 0 ? 2 : 0)
                    ...
                    + (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_SUPER_LUCK) ? 1 : 0)
    ...
    if (critChance != CRITICAL_HIT_BLOCKED
     && (BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_BATTLE_ARMOR) || BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_SHELL_ARMOR)))
    {
        enum Ability blocker = BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_BATTLE_ARMOR) ? ABILITY_BATTLE_ARMOR : ABILITY_SHELL_ARMOR;
...
s32 CalcCritChanceStageGen1(struct BattleContext *ctx)
{
    ...
    if (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_SUPER_LUCK))
        critChance *= 4;

    if (critChance > 255)
        critChance = 255;

    if (BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_BATTLE_ARMOR) || BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_SHELL_ARMOR))
    {
        enum Ability blocker = BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_BATTLE_ARMOR) ? ABILITY_BATTLE_ARMOR : ABILITY_SHELL_ARMOR;
...
s32 GetAdjustedDamage(struct BattleContext *ctx, s32 damage)
{
    ...
    else if (GetConfig(B_STURDY) >= GEN_5 && BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_STURDY) && IsBattlerAtMaxHp(ctx->battlerDef))
    {
        enduredHit = TRUE;
...
static inline void MulByTypeEffectiveness(struct BattleContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    else if ((ctx->moveType == TYPE_FIGHTING || ctx->moveType == TYPE_NORMAL) && defType == TYPE_GHOST && gBattleMons[ctx->battlerDef].volatiles.foresight && mod == UQ_4_12(0.0))
    {
        ...
        && (BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_SCRAPPY) || BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_MINDS_EYE))
        && mod == UQ_4_12(0.0))
    {
        enum Ability ability = BattlerHasAbilityForCalc(ctx->battlerAtk, ctx->abilityAtk, ABILITY_SCRAPPY) ? ABILITY_SCRAPPY : ABILITY_MINDS_EYE;
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct BattleContext *ctx, uq4_12_t modifier)
{
    ...
    if (((BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_WONDER_GUARD) && modifier <= UQ_4_12(1.0) && !isPresentHealing)
        || (BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_TELEPATHY) && ctx->battlerDef == BATTLE_PARTNER(ctx->battlerAtk)))
        && GetMovePower(ctx->move) != 0)
    {
        enum Ability ability = BattlerHasAbilityForCalc(ctx->battlerDef, ctx->abilityDef, ABILITY_WONDER_GUARD) ? ABILITY_WONDER_GUARD : ABILITY_TELEPATHY;
...
bool32 CanMoveSkipAccuracyCalc(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum Move move, enum ResultOption option)
{
    ...
    else if (BattlerHasAbilityForCalc(battlerDef, abilityDef, ABILITY_NO_GUARD)
          && (moveEffect != EFFECT_SKY_DROP || gBattleStruct->skyDropTargets[battlerDef] == SKY_DROP_NO_TARGET))
    {
        effect = TRUE;
        ability = ABILITY_NO_GUARD;
