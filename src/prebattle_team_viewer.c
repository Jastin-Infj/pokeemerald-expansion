...
static u8 CountPreviewParty(struct Pokemon *party);
...
bool32 PreBattleTeamViewer_LoadCachedOpponentParty(void)
{
    ...
    memcpy(gParties[B_TRAINER_OPPONENT_A], sTeamViewerState.enemyParty, sizeof(sTeamViewerState.enemyParty));
...
static bool32 PrepareOpponentCache(void)
{
    ...
    sTeamViewerState.enemyPartyCount = CountPreviewParty(sTeamViewerState.enemyParty);
...
static u8 CountPreviewParty(struct Pokemon *party)
{
    ...
    while (count < PARTY_SIZE && GetMonData(&party[count], MON_DATA_SPECIES) != SPECIES_NONE)
        count++;
...
{
    DrawHeader();
    DrawTeamPanel(WIN_PLAYER, gParties[B_TRAINER_PLAYER], sTeamViewerState.playerCount, TEAM_VIEWER_SIDE_PLAYER);
...
static void RefreshTeamCursor(bool8 redrawFooter)
{
    DrawTeamLabels(WIN_PLAYER, gParties[B_TRAINER_PLAYER], sTeamViewerState.playerCount, TEAM_VIEWER_SIDE_PLAYER);
...
static struct Pokemon *GetSelectedMon(void)
{
    ...
    party = sTeamViewerState.side == TEAM_VIEWER_SIDE_PLAYER ? gParties[B_TRAINER_PLAYER] : sTeamViewerState.enemyParty;
...
static void CreateTeamIcons(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (i < sTeamViewerState.playerCount)
            CreateTeamIcon(gParties[B_TRAINER_PLAYER], TEAM_VIEWER_SIDE_PLAYER, i);
