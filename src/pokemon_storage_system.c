...
static enum Species GetDisplayMonGfxSpecies(void);
...
static void CreatePartyMonsSprites(bool8 visible)
{
    ...
    bool32 isEgg = PokemonVendor_ShouldDisplayMonAsEgg(&gParties[B_TRAINER_PLAYER][0]);
    ...
    for (i = 1; i < PARTY_SIZE; i++)
    {
        ...
        isEgg = PokemonVendor_ShouldDisplayMonAsEgg(&gParties[B_TRAINER_PLAYER][i]);
        if (species != SPECIES_NONE)
        {
