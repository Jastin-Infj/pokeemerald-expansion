...
Current normal save layout is two rotating 14-sector save slots:
...
- reclaim `SAVE_BLOCK_3_CHUNK_SIZE` by removing the SaveBlock3 area (`SECTOR_DATA_SIZE`
  can become 4084, but DexNav search levels and other SaveBlock3 features must move
  or be disabled);
- increase `NUM_SECTORS_PER_SLOT` from 14 to 15 or 16, consuming special sectors
  such as Hall of Fame / Trainer Hill / Recorded Battle while keeping two save slots;
- shrink another normal-save payload such as PokemonStorage;
- create custom storage in special sectors with its own checksum / mirroring policy.

The relevant barrier is physical sector allocation and save-format compatibility,
not `u8` vs `u16` by itself. Widening integer types is still required for bag lists
above 255 entries, but it does not create save capacity.
...
**Bag expansion note**: 通常 bag は `SaveBlock3` ではなく `SaveBlock1` 内の
`struct Bag`。現行の `struct Bag` は `0x2E8` bytes で、1 item slot 追加ごとに
約 4 bytes の SaveBlock1 増加になる。`test/save.c` 上の SaveBlock1 余りは
304 bytes なので、Key Items 30 -> 64 のような小型拡張は容量上は収まるが、
save layout は変わる。TM/HM 64 -> 250-300 のような大型拡張は current spare を超える。
詳細は `docs/features/bag_expansion/README.md`。
...
|---|---|---|
| DexNav species search levels | 既存 `SaveBlock3.dexNavSearchLevels` | 既存 config (`USE_DEXNAV_SEARCH_LEVELS`)。現行 `NUM_SPECIES = 1573` なので SaveBlock3 をほぼ使い切る。 |
...
| Champions Challenge state (active/status/saved-flags) | **SaveBlock1 dedicated struct** | 6 匹分の `struct Pokemon` snapshot (約 600 B total) と bag snapshot (現状 `0x2E8` B) を含む可能性が高い。SaveBlock3 (1624 B 上限) には向かない。SaveBlock1 で `FREE_MYSTERY_GIFT` などを切って空き容量を確保する。 |
| bag_expansion | **SaveBlock1 `struct Bag`** | `BAG_*_COUNT` fixed arrays | 通常 bag capacity は SaveBlock1 layout change。SaveBlock3 の空きは通常 bag には使わない。 |
| unified_move_relearner virtual TM unlocks | SaveBlock1/2/3 dedicated bitset | 250-300 bits if story-gated | Always-available virtual TM pool needs no save data. If unlock state is required, use compact bitset state rather than bag slots. |
...
| Party / Status UI skin | compile-time config first | `2 x 3` party grid / BW Summary などの visual UI は SaveBlock を使わない方針。Classic / BW を player option 化する場合だけ SaveBlock2 option field を検討する。Champions run-session state とは共有しない。 |
...
現行 `struct Pokemon[PARTY_SIZE]` は約 600 B、現行 `struct Bag` は `0x2E8`
(744) B なので、sketch の主要 snapshot は約 1.4 KB + small metadata。
SaveBlock1 の current spare 304 B には収まらないが、`FREE_MYSTERY_GIFT`
(876 B) + `FREE_MYSTERY_EVENT_BUFFERS` (1104 B) = 1980 B を空ければ余裕が出る。
...
| bag_expansion | SaveBlock1 `struct Bag` | `BAG_*_COUNT` 変更 | 1 slot 約 4 B。1000 total slots / TM-HM 350 slots は約 +3256 B。current spare 304 B + SaveBlock1 `FREE_*` 2516 B だけでは約 436 B 不足。DexNav を残すなら SaveBlock3 chunk reclaim ではなく、15-sector normal save / PokemonStorage shrink / special-sector custom storage 等が必要。 |
...
champions_challenge と bag_expansion はどちらも SaveBlock1 を消費するため、
着手前にこの doc と `docs/features/bag_expansion/` を再確認する。
in-place swap は通常 party snapshot を失うリスクがあるため、保存する state の量を
小さくするか、明示的な dedicated snapshot / migration 方針を決める。
