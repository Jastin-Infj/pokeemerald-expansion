...
| `FREE_TRAINER_TOWER` | 不明 | SaveBlock1 |
...
サイズは 4-5 KB 級。SaveBlock1 の `FREE_MYSTERY_GIFT` (876 B) + `FREE_MYSTERY_EVENT_BUFFERS` (1104 B) を空けても足りない可能性が高い。
**MVP では party / bag snapshot 領域の縮約 (バッグはカテゴリ単位の slot 数を上限化、party は live `gPlayerParty` を別 buffer に置かず in-place swap) を検討する。**
...
- `FREE_TRAINER_TOWER` の解放サイズが `include/config/save.h` のコメントに無い (`x bytes`)。実測で埋める。
