...
| Bag / item storage | `include/constants/global.h`, `include/global.h`, `src/item.c`, `src/item_menu.c`, `src/load_save.c`, `src/rom_header_gf.c`, `test/save.c` | `BAG_*_COUNT`、`struct Bag`、SaveBlock1 layout、bag UI buffers、ROM header bag counts、save compatibility |
...
git diff OLD_REV..NEW_REV -- include/constants/global.h include/global.h src/item.c src/item_menu.c src/load_save.c src/rom_header_gf.c test/save.c
