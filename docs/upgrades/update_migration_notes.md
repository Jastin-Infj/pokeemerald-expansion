...
- `RHH` は fetch 専用。push URL は `DISABLED` に固定する。
- upstream release / tag / changelog の確認は `RHH` から fetch してよい。
- 作業 branch の push は `origin` のみ。
- PR を作る場合は `--repo Jastin-Infj/pokeemerald-expansion` を明示し、`rh-hideout/pokeemerald-expansion` を base repo にしない。
