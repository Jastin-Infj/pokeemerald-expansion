...
|---|---|---|
| docs-only | `docs/` と、必要な workflow-only `AGENTS.md` だけを更新する。 | `master` へ取り込み候補にしてよい。 |
| new implementation | current `master` には存在しない source / include / data / tools / generated 差分を新しく作る。 | `feature/*` / `integration/*` に置き、`master` へ直接 merge しない。 |
...
| docs handoff | runtime 実装は持ち込まず、branch evidence、採否判断、残リスクだけを docs に残す。 | docs-only branch で `master` へ取り込み候補にしてよい。 |
