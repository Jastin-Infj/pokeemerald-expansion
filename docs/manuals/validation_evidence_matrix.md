...
| Trainer Battle Party Selection | `feature/battle-selection-mvp`; adopted into `integration/runtime-dev-20260529` through Team Viewer phase2 | [test_plan](../features/battle_selection/test_plan.md) | Branch docs record build/manual evidence. Integration local `all`, `debug`, full `check`, docs build, and short-party follow-up boot smoke passed. | No dedicated automated short-party selection test yet. | User manual validation for single/double/party restore recorded. User confirmed integrated short-party behavior on 2026-05-31. | Keep default double minimum at 2; 1v2 double remains optional experiment only. |
