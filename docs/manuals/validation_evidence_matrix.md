...
| Battle Item Restore Policy | `feature/battle-item-restore-current-master-20260519`; historical shelf #14 / `feature/battle-item-restore-policy` | [test_plan](../features/battle_item_restore_policy/test_plan.md) | 2026-05-19 `all`, `debug`, full `check`, and focused `check` routes passed. | Direct restore and full Oran Berry consume/restore tests passed. | mGBA Live boot/input smoke passed and cleaned up; feature-specific behavior is covered by headless full battle test. | Default `TRUE` is adopted on the current feature branch; `master` remains docs-only / source-free. |
