...
|---|---|---|---|---|---|
| #68 | Runtime integration staging | Yes | `integration/runtime-dev-20260529` | `UNSTABLE` | Local `all` / `debug` / focused feature checks pass; after switching All Ability Slots default to `TRUE`, full `check` is expected-red on upstream single-ability assumptions; Actions not re-waited |
...
| #60 | All Ability Slots shelf | Yes | `feature/all-ability-slots-runtime-20260523`; default-TRUE follow-up on `integration/runtime-dev-20260529` 2026-05-31 | `UNSTABLE` | Adopted into #68; runtime integration now sets `B_ALL_ABILITY_SLOTS TRUE` so normal battles use all active slots |
...
| 2026-05-31 | All Ability Slots default TRUE follow-up | Mixed / expected-red full suite | `B_ALL_ABILITY_SLOTS` now defaults to `TRUE` in `include/config/battle.h`, so `BattleStartClearSetData()` enables all-slot mechanics for normal battles, not only debug routes. `rtk make -j16 -O check TESTS='All Ability Slots'`, `rtk make -j16 -O all`, `rtk make -j16 -O debug`, docs build, and mGBA Live boot session `all-ability-default-true-20260531` pass. Full `rtk make -j16 -O check` now fails with 264 failed / 4289 passed / 5203 total because upstream tests still encode single-ability default expectations; log: `/tmp/all-ability-global-true-full-check.log`. |
