...
| 種類 | 主な編集元 | 表示入口 | 注意点 |
| --- | --- | --- | --- |
| Battle message | `include/constants/battle_string_ids.h`, `src/battle_message.c`, `data/battle_scripts_1.s`, `data/battle_scripts_2.s` | `printstring`, `printfromtable`, `resultmessage`, `critmessage` | animation、HP bar、move-end hook と順番が絡む |
...
| Field / map message | `data/maps/*/scripts.inc`, `src/data/text/*.h`, `src/strings.c`, feature-local C files | field script `message`, C `ShowFieldMessage` | `waitmessage` / `closemessage` は field script の待ち |
| Braille / special window | `data/maps/*/scripts.inc`, `src/scrcmd.c` | `braillemessage`, custom C window | 通常 textbox と layout が違う |
