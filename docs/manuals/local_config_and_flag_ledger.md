...
| `B_ALL_ABILITY_SLOTS_MOLD_BREAKER` / `B_ALL_ABILITY_SLOTS_NEUTRALIZING_GAS` | Branch-only battle config | `docs/features/all_ability_slots/` | Not present on `master`。 | Balance knobs for whether non-representative Mold Breaker-family abilities bypass target abilities and whether non-representative Neutralizing Gas suppresses other abilities. Integration currently leaves both enabled when all-slot mode is enabled. |
| `P_SUMMARY_SCREEN_ALL_ABILITY_SLOT_SWITCH` | Branch-only Summary config | `docs/features/all_ability_slots/` | Not present on `master`。 | PR #60 and runtime integration default `TRUE`; it only controls `L` / `R` cycling of the displayed ability slot on the Info page when `B_ALL_ABILITY_SLOTS` is enabled. |
...
| `B_RESTORE_HELD_BATTLE_BERRIES` | Branch-only battle config | `docs/features/battle_item_restore_policy/` | Not present on `master`。 | Branch evidence uses default `TRUE` per user direction. Reconfirm if adopting a conservative integration default. |
| `I_HELD_ITEM_CATALOG_ASSIGNMENT` | Branch-only item config | `docs/features/nonconsumable_held_items/` | Not present on `master`。 | Implemented on `feature/held-item-catalog-current-master-20260519`, default `TRUE`: non-mail, non-Key held-effect items act as one unique Bag token for assignment; ordinary no-hold-effect items remain physical. |
| `SAVE_CHAMPIONS_RUN_SESSION` | Branch-only save config | `docs/features/champions_challenge/` | Not present on `master`。 | Adopted on `integration/runtime-dev-20260529`, default `TRUE`; adds `SaveBlock1.championsRun` and requires `FREE_MYSTERY_EVENT_BUFFERS` plus `FREE_MYSTERY_GIFT` to fit the normal-state snapshot. Focused SaveBlock test expects SaveBlock1 size `15664`. |
| `CHAMPIONS_RUN_ENTRY_PARTY_MODE` | Branch-only save / facility config | `docs/features/champions_challenge/` | Not present on `master`。 | Default `CHAMPIONS_RUN_START_PARTY_EMPTY`; alternatives copy the last clear party from PC or copy the current normal party into the run. |
