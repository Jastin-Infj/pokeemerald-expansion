...
- Aggressive gimmick pressure: spends Dynamax, Tera, or a damaging Z-Move on a configured 70% pressure route while reserves still exist.
- Read-player-move debug prediction: with `AI_FLAG_READ_PLAYER_MOVE`, spends Dynamax into an already confirmed `Fake Out` even without the ordinary prediction flag.
...
- 2026-06-06 `rtk make -j16 -O check TESTS='AI_FLAG_AGGRESSIVE_GIMMICK|AI_FLAG_READ_PLAYER_MOVE|AI_FLAG_GIMMICK_ENV_DYNAMAX_TERA|AI_FLAG_GIMMICK_ENV_ALL'`: pass. Covers the aggressive 70% Dynamax / Tera / Z-Move pressure route, the read-player-move Fake Out Dynamax regression, combined Dynamax / Tera fallback, and existing all-gimmick environment checks.
