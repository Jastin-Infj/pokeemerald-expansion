...
- Bench Pokemon can be active board resources, not only replacements after losing a matchup. This update maps that tournament pattern to entry weather / terrain setters (`Drizzle`, `Drought`, `Sand Stream`, `Snow Warning`, terrain surges) and speed-control setters (`Tailwind`, `Trick Room`) when they can flip the board or increase pressure.
