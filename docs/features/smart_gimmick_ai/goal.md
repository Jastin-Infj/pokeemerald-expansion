...
- Battle action log v3 reason tags plus compact threat / risk trace fields for non-obvious AI choices such as hax-out, desperation comeback, Perish escape, ally sacrifice, and Commander slot correction.
...
- Partial: debug reason tags plus compact threat / risk trace fields in `BattleActionLog` v3 and the mGBA Live JSON exporter. Candidate scores, rejected alternatives, and full short-horizon facts are not logged yet.
- Partial: short-horizon evaluation covers Perish Song, selected setup, mode-loss pressure, reserve-entry sacrifice, Protect counterplay including Feint / bypass / selected Z or Max chip, and clean damage suppression, but it is not a full candidate-line solver.
