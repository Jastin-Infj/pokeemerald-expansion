...
| Ability popup / form-change focused checks | `rtk make -j16 -O check TESTS='Mega Evolution'`; `rtk make -j16 -O check TESTS='Pressure'`; `rtk make -j16 -O check TESTS='Delta Stream'`; `rtk make -j16 -O check TESTS='Terastallization'`; `rtk make -j16 -O check TESTS='Ultra Burst'`; `rtk make -j16 -O check TESTS='Primal Reversion'` | Pass on 2026-06-06 after the ability-popup refresh / form-change cleanup fix. Sequential results: Mega Evolution 5 tests, Pressure 5 tests, Delta Stream 1 test, Terastallization 3 tests, Ultra Burst 5 tests, Primal Reversion 15 tests. Earlier parallel attempts are non-evidence because separate `make check` processes collided on the same test ELF / tmpfd artifacts. |
...
| Normal ROM build | `rtk make -j16 -O all` | Pass on 2026-06-06 after the ability-popup refresh / form-change cleanup fix. |
| Docs build | `rtk mdbook build docs` | Pass on 2026-06-06 after documenting the ability-popup refresh / form-change cleanup fix, with existing warnings: missing root `CHANGELOG.md` include, existing `CREDITS.md` `</img>` warning, large search index. |
| mGBA Live smoke | Boot current ROM and capture one screenshot / input state | Pass on 2026-06-06. For the ability-popup refresh / form-change cleanup fix, sandboxed CLI startup first failed because it could not create `~/.mgba-live-mcp/runtime/sessions/ability-popup-formchange-20260606`; rerunning with approval and `DISPLAY=:0` booted `pokeemerald.gba` to the title screen, exported `/tmp/ability-popup-formchange-20260606.png`, `mgba-live-cli stop` returned `stopped:true`, and `status --all` returned `[]`. This was a boot smoke only; the Mega Mewtwo / Delta Stream visual sequence still needs manual progression from a debug-enabled gauntlet save. |
