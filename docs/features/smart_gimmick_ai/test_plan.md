...
| mGBA Live smoke | Boot current ROM and capture one screenshot / input state | Pass on 2026-06-05. Direct script-build binary failed without `DISPLAY`; wrapper `/home/jastin/.local/bin/mgba-qt` booted `pokeemerald.gba` to the title screen. `mgba_live_stop` returned `stopped:true`; CLI `status --all` returned `[]`. |
