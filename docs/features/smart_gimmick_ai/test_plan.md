...
| Focused aggressive/read gauntlet AI tests | `rtk make -j16 -O check TESTS='AI_FLAG_AGGRESSIVE_GIMMICK\|AI_FLAG_READ_PLAYER_MOVE\|AI_FLAG_GIMMICK_ENV_DYNAMAX_TERA\|AI_FLAG_GIMMICK_ENV_ALL'` | Pass on 2026-06-06. Covers aggressive Dynamax / Tera / Z-Move pressure use, read-player-move Fake Out Dynamax, combined Dynamax / Tera fallback, and all-gimmick environment checks. |
...
| Normal ROM build | `rtk make -j16 -O all` | Pass on 2026-06-06 after the ability-popup refresh / form-change cleanup fix, after the shared ability-popup palette ownership fix, and after the debug opponent gimmick-bit preservation / Kyogre spread-pressure fix. |
...
- Gauntlet AI trainers use `Smart Trainer`, `Prediction`, `Smart Gimmick`, `Smart Switching`, `Smart Mon Choices`, `Omniscient`, `Know Opponent Party`, `Powerful Status`, `Aggressive Gimmick`, and `Read Player Move`. This is the "AI can read with full information plus already-confirmed player move input" route for checking whether predictions, pivots, and gimmick timing are actually good enough when the opposing team is known.
...
- `AI_FLAG_READ_PLAYER_MOVE` is a debug/testing input-read layer, not a fair normal-NPC policy. It improves full-information gauntlet audits, but it can still be partial in doubles because the opponent may recompute after one player-side command is confirmed and before the other is confirmed.
