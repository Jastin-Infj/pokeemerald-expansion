...
| Ability popup / form-change focused checks | `rtk make -j16 -O check TESTS='Mega Evolution'`; `rtk make -j16 -O check TESTS='Pressure'`; `rtk make -j16 -O check TESTS='Delta Stream'`; `rtk make -j16 -O check TESTS='Terastallization'`; `rtk make -j16 -O check TESTS='Ultra Burst'`; `rtk make -j16 -O check TESTS='Primal Reversion'`; `rtk make -j16 -O check TESTS='Drizzle'`; `rtk make -j16 -O check TESTS='Skill Swap'` | Pass on 2026-06-06 after the ability-popup refresh / form-change cleanup fix. Sequential results: Mega Evolution 5 tests, Pressure 5 tests, Delta Stream 1 test, Terastallization 3 tests, Ultra Burst 5 tests, Primal Reversion 15 tests. After the normal ability-popup slide-out cleanup, Drizzle 5 tests, Pressure 5 tests, Delta Stream 1 test, and Skill Swap 4 tests pass. Earlier parallel attempts are non-evidence because separate `make check` processes collided on the same test ELF / tmpfd artifacts. |
...
| Normal ROM build | `rtk make -j16 -O all` | Pass on 2026-06-06 after the ability-popup refresh / form-change cleanup fix and after the normal ability-popup slide-out cleanup. |
| Docs build | `rtk mdbook build docs` | Pass on 2026-06-06 after documenting the ability-popup refresh / form-change cleanup fix and normal ability-popup slide-out cleanup, with existing warnings: missing root `CHANGELOG.md` include, existing `CREDITS.md` `</img>` warning, large search index. |
| mGBA Live smoke | Boot current ROM and capture one screenshot / input state | Pass on 2026-06-06. For the ability-popup refresh / form-change cleanup fix, sandboxed CLI startup first failed because it could not create `~/.mgba-live-mcp/runtime/sessions/ability-popup-formchange-20260606`; rerunning with approval and `DISPLAY=:0` booted `pokeemerald.gba` to the title screen, exported `/tmp/ability-popup-formchange-20260606.png`, `mgba-live-cli stop` returned `stopped:true`, and `status --all` returned `[]`. For the normal ability-popup slide-out cleanup, the same sandbox write failure occurred for `ability-popup-drizzle-cleanup-20260606`; rerunning with approval booted `pokeemerald.gba`, exported `/tmp/ability-popup-drizzle-cleanup-20260606.png`, `stop` returned `stopped:true`, and `status --all` returned `[]`. Both were boot smokes only; the Mega Mewtwo / Delta Stream and Kyogre / Drizzle visual sequences still need manual progression from a debug-enabled gauntlet save. |
...
- Ability popups should not linger over form-change messages, move animations, or follow-up weather messages. In Mega gauntlets, a switch-in `Pressure` popup from `Mewtwo` should close before the `Mewtwonite Y is reacting` Mega Evolution message, and repeated same-battler ability activations should refresh the existing popup instead of leaving black OBJ-tile rectangles. In double battle weather leads, a `Kyogre's Drizzle` popup should finish its slide-out before the opposing move animation and before the "Rain continues to fall" message.
