...
| Focused smart Z tests | `rtk make -j16 -O check TESTS='AI_FLAG_SMART_Z_MOVE'` | Pass on 2026-06-06; 3 tests passed. |
...
| Debug trainer fixture generation | `rtk make tools/trainerproc/trainerproc`; `rtk make -j16 -O debug` | Pass on 2026-06-06. Regenerated `.party` trainer data and built the debug ROM path with Z-Power / Dynamax access fixture data. `rtk make -j16 -O debug` also passed after the double-switch execution guard. |
...
| Normal ROM build | `rtk make -j16 -O all` | Pass on 2026-06-06, including after the double-switch execution guard. |
...
| mGBA Live smoke | Boot current ROM and capture one screenshot / input state | Pass on 2026-06-06. MCP startup without `DISPLAY` previously failed with Qt `xcb` display initialization. For the double-switch guard handoff, sandboxed CLI startup first failed because it could not create `~/.mgba-live-mcp/runtime/sessions/...`; rerunning with approval and `DISPLAY=:0` booted `pokeemerald.gba` in session `smart-gimmick-switch-guard-20260606`, exported `/tmp/smart-gimmick-switch-guard-20260606.png`, `mgba-live-cli stop` returned `stopped:true`, and `status --all` returned `[]`. This was a boot smoke only; the exact debug double-battle ASSERT path still needs manual progression to the Party debug fixture. |
...
- Opponent gimmicks are not forced to fire immediately. They are expected to activate only when the smart timing checks find a concrete payoff: last-Pokemon pressure, KO conversion, Max Move board control, disruption prevention, useful Mega / Tera form timing, or a valid Z-Move payoff.
