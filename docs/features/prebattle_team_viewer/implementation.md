...
2026-06-02 party-reshow follow-up: the remaining user-provided screenshots
`pokeemerald-18.png` through `pokeemerald-22.png` showed `TEAM INFO` during the
player switch-out / switch-in messages (`Good job... Come back!`, `Go! ...`),
not only on the shift yes/no prompt. The root cause was the unconditional
`TryShowTeamViewerActionHint()` call at the end of `ReshowBattleScreenAfterMenu()`;
party-menu returns share that reshow path while battle script text is still running.
The fix removes that post-reshow recreation and keeps hint creation on the actual
action-menu draw / Team Viewer return path.
...
| `include/battle_controllers.h`, `src/battle_controller_player.c`, `src/reshow_battle_screen.c` | Adds the in-battle viewer shortcut, post-reshow action prompt/menu redraw, held-key release guard after closing the viewer, and MoveInfo-aligned double `TEAM INFO` hint positioning. The 2026-06-02 party-reshow follow-up removes unconditional post-reshow hint recreation so party switch text cannot inherit the action-menu hint. |
...
| 2026-06-02 party reshow switch-message cleanup | Pass | User reference screenshots `pokeemerald-18.png` through `pokeemerald-22.png` showed `TEAM INFO` during switch-out / switch-in battle text, not only the script yes/no prompt. `rtk git diff --check`, `rtk make -j16 -O all`, `rtk make -j16 -O debug`, and `rtk make -j16 -O check` passed with the existing RWX linker warning. mGBA Live session `teaminfo-party-reshow-20260602` used `Party -> Selection Battle`, selected 3/3, entered battle, confirmed action-menu `TEAM INFO`, switched to Gardevoir and then Aggron, and confirmed the `Go! Aggron!` live view at frame `584038` had no `TEAM INFO` sprite while the Team Viewer still reopened from the later action menu. Evidence: `/tmp/teaminfo-party-reshow-action-menu-20260602.png`, `/tmp/teaminfo-party-reshow-after-viewer-return-20260602.png`; stop returned `alive_after:false`. |
