...
| Debug validation route | Pass | Added debug-only `Party -> Selection Battle` and `Party -> Selection Double`, which give the player six mons with moves and start repeatable single / double trainer routes that exercise viewer -> selection -> battle. `Party -> Start Debug Battle` bypasses normal trainer setup and is not a valid Trainer Battle Selection check. |
...
| 2026-06-02 shift prompt script yes/no cleanup | Pass | Reported screenshot showed `TEAM INFO` still visible after the opponent sent the next Pokemon and the shift yes/no prompt appeared. Source audit found this prompt uses `Cmd_yesnobox()` directly, bypassing `PlayerHandleYesNoBox()` and the previous 2026-05-31 cleanup. Fix exposes `TryDestroyTeamViewerActionHint()` and calls it before drawing the script yes/no window. `rtk git diff --check`, `rtk make -j16 -O all`, `rtk make -j16 -O debug`, and `rtk make -j16 -O check` passed with the existing RWX linker warning. mGBA Live session `teaminfo-shift-prompt-20260602` used `Party -> Selection Battle`, selected 3/3, entered Gabrielle battle, picked a move from an action menu with `TEAM INFO` visible, KO'd Skitty, and confirmed both the next-Pokemon shift prompt and the follow-up party menu had no `TEAM INFO` sprite. Evidence: `/tmp/teaminfo-shift-prompt-clean-20260602.png`, `/tmp/teaminfo-shift-party-menu-clean-20260602.png`; stop returned `alive_after:false`. |
| 2026-06-02 party reshow switch-message cleanup | Pass | User reference screenshots `pokeemerald-18.png` through `pokeemerald-22.png` showed the remaining issue during switch-out / switch-in battle text (`Good job... Come back!`, `Go! ...`). Root cause was unconditional `TryShowTeamViewerActionHint()` at the end of `ReshowBattleScreenAfterMenu()`, which also runs when returning from the party menu while battle script text is still active. Fix removes that post-reshow hint recreation and leaves hint creation to the actual action-menu draw / Team Viewer return path. `rtk git diff --check`, `rtk make -j16 -O all`, `rtk make -j16 -O debug`, and `rtk make -j16 -O check` passed with the existing RWX linker warning. mGBA Live session `teaminfo-party-reshow-20260602` used `Party -> Selection Battle`, selected 3/3, entered battle, confirmed action-menu `TEAM INFO`, switched to Gardevoir and then Aggron, and confirmed the `Go! Aggron!` live view at frame `584038` had no `TEAM INFO` sprite while `R -> B` Team Viewer return still restored the action-menu hint. Evidence: `/tmp/teaminfo-party-reshow-action-menu-20260602.png`, `/tmp/teaminfo-party-reshow-after-viewer-return-20260602.png`; stop returned `alive_after:false`. |
