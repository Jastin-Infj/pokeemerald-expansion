...
2026-05-09 の catalog 確認では、現行 item 定義を全種類そのままバッグに
持てるようにすると、追加 687 slots / 約 2748 bytes が必要になる。これは
SaveBlock1 の現余り 304 bytes を大きく超え、SaveBlock1 sector budget も
約 2444 bytes 超過する。
...
| TM Shop Migration / TM Expansion | `BAG_TMHM_COUNT 64` は 250 TM を全部 item slot として持つには不足する。300 slots でも SaveBlock1 と `u8` ROM header count に引っかかり、2300 slots は `BagPocket.capacity:10` も超える。virtual TM registry か TM/HM pocket 拡張を決める必要がある。 |
