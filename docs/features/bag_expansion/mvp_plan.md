...
4. Decide whether the implementation accepts SaveBlock1 `FREE_*` plus SaveBlock3 chunk reclaim. `FREE_*` alone reaches only about 891 total slots.
...
| 8 | `include/save.h`, `src/save.c` | If targeting 1000 slots, decide whether to reclaim SaveBlock3 chunk bytes by changing the save sector format. |
...
- Optional SaveBlock3 chunk reclaim / `SECTOR_DATA_SIZE` policy if the full 1000-slot target remains raw bag storage.
