...
| Item duplication through `TAKE` | High | Catalog assignment can create infinite physical Bag copies if Take still calls `AddBagItem`. | Implemented helper adds no copy when the Bag already owns the item; it adds one copy only for first-time held item acquisition. |
| Item deletion through `GIVE` | High | If Give still removes from Bag while multiple mons are allowed, the feature does not solve quantity friction. | Implemented helper keeps Bag quantity unchanged for catalog-owned held-effect items. |
| Catalog scope too broad | High | Applying unique-token add behavior to ordinary consumables, Poke Balls, or utility items would break normal inventory economy. | Catalog policy now requires a real hold effect and leaves no-hold-effect items physical. |
...
| PC Storage bypass | High | Storage item mode can move held items outside the party menu callbacks. | Storage give / take / close / release return paths now use catalog-aware helpers; mGBA UI validation remains required. |
...
| Toss ambiguity | Medium | Toss can become "delete catalog item" or just "clear held item" depending on UI wording. | Bag Toss is blocked for catalog tokens; Party Toss still clears the Pokemon held slot. |
...
## Accepted Risk For Catalog Runtime Slice

The catalog slice solves Bag quantity friction, but it does not change
battle-time consumption or battle-end restoration. Use Battle Item Restore Policy
PR #47 or a future integration branch for consumed battle items.
