...
| Item deletion through `GIVE` | High | If Give still removes from Bag while multiple mons are allowed, the feature does not solve quantity friction. | Implemented helper keeps Bag quantity unchanged for catalog-owned non-mail items. |
