...
Code status: Runtime slice committed on `feature/field-move-modernization-mvp`, stacked itemization on `feature/field-move-toolkit-item`
...
2026-05-09 の手動確認で Field Kit itemization と debug shortcut の挙動は期待どおりと確認済み。次の UI 方針は、Key Items pocket で `ITEM_FIELD_KIT` を「使う」操作を Fly 導線として扱うこと。Field Kit が Fly launcher を兼ねれば、HM 技所持に依存しない進行の入口を Key Item に集約できる。Teleport / Dig など非 HM field move はこの方針には含めず、既存挙動のままにする。
...
- Field Kit item use = Fly launcher は次 slice 方針として記録し、この branch では実装しない。
