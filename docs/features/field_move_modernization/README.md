...
`feature/field-move-toolkit-item` では単一 Key Item の `ITEM_FIELD_KIT` を追加し、Emerald の HM 入手イベントを Field Kit upgrade に差し替える。古い HM receive flags は TM Shop Migration で `FLAG_UNUSED_0x...` に戻したため、capability flags は field move feature 側で別名 / 別枠として用意する。初期値では Field Kit 所持 + capability flag + 既存 badge gate をすべて要求する。Field Kit 配布に失敗した場合は capability flag を立てず、既存の bag full message で止める。アイコンは `field_styler.png` / `field_styler.pal` の専用 asset に差し替え済み。

2026-05-09 の手動確認で Field Kit itemization と debug shortcut の挙動は期待どおりと確認済み。その後、Key Items pocket で `ITEM_FIELD_KIT` を「使う」操作と SELECT registered key item 起動を Field Kit menu にした。Fly は Field Kit + Fly capability + badge gate を満たす時だけ先頭に表示し、Teleport / Dig は既存 map gate に従う utility shortcut として続けて表示する。Field Kit menu の枠高さ、標準 window border palette、Teleport / Dig の直開始、Fly の夜 palette fade も手動確認済み。

Field Kit item icon / palette の差し替え手順と検証観点は [Field Kit Icon And Palette Manual](../../manuals/field_kit_icon_palette_manual.md) に分離した。

per-HM item 方式は Key Items pocket を圧迫し、badge-only は story pacing の自由度が低い。Key Items pocket は現状 `BAG_KEYITEMS_COUNT 30` の固定長なので、[Bag Expansion](../bag_expansion/README.md) は別 feature の大型改修として扱い、この field move feature では実装しない。
