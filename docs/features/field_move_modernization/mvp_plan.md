...
| Field Kit | `feature/field-move-toolkit-item` で単一 Key Item `ITEM_FIELD_KIT` を追加。capability flags は field move feature 側で別名 / 別枠として用意する。 |
...
- Key Items pocket は現状 `BAG_KEYITEMS_COUNT 30` なので、key item 方式の前に bag capacity / save layout / debug grant の改修が必要。[Bag Expansion](../bag_expansion/README.md) は別 feature の大型改修として分離する。
...
- [Bag Expansion](../bag_expansion/README.md) は別 feature の大型改修で扱い、この feature は単一 key item 以上の bag pressure を前提にしない。
