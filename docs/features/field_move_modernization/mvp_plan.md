...
Status: Initial MVP implemented; Field Kit itemization user-confirmed
Code status: Runtime slice on `feature/field-move-modernization-mvp`, Field Kit itemization on `feature/field-move-toolkit-item`
最終更新: 2026-05-09
...
| Field Kit use | 次 slice 方針として、Key Items pocket で `ITEM_FIELD_KIT` を使う操作を Fly launcher / region map 入口にする。Teleport / Dig は対象外。 |
...
- Field Kit の bag field use は Fly launcher として扱い、Fly の UI 入口を Key Item に集約する。
