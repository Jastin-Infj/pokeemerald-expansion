...
- Field Kit route: each Emerald field move upgrade gives `ITEM_FIELD_KIT` only once, sets the matching capability flag, and only permits use when Field Kit + capability + badge gate are all satisfied.
- Debug shortcut route: debug menu `Scripts... > Field Kit Full` gives the Field Kit, sets all HM capability flags, and sets all badges for quick manual field move validation. `Field Kit Item`, `Field Kit Flags`, and `Field Kit Clear` are available for focused gate checks.
