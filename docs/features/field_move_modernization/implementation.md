...
- Next open polish item is visual only: replace the temporary `gItemIcon_QuestionMark` / `gItemIconPalette_QuestionMark` placeholder with a Field Kit-specific icon and palette. Use [Field Kit Icon And Palette Manual](../../manuals/field_kit_icon_palette_manual.md) for that handoff.
