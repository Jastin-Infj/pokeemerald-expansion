...
- Fly は Field Kit + `FLAG_RECEIVED_HM_FLY` + badge gate を満たす場合だけ menu に表示する。Fly 選択時は region map を直接開き、B cancel では party menu ではなく field に戻る。
...
- Manually confirm Field Kit menu from Key Items pocket after `Field Kit Full`: expected order is Fly / Teleport / Dig, with Fly colored red.
- Manually confirm Field Kit registered to SELECT opens the same menu.
- Manually confirm Fly B cancel returns to the field and that invalid Teleport / Dig maps keep the cannot-use message without locking controls.
- Manually confirm Teleport / Dig from Field Kit do not show the brief return-to-field brightness flash on night maps.
- Confirmed manually on 2026-05-09: Fly-unavailable Field Kit menu uses a 2-row frame, and cave / dark-map Field Kit menu uses the standard black window frame palette.
- Manually confirm Fly from Field Kit no longer brightens the night field before the region map opens.
