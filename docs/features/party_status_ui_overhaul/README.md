...
5. Validate field party menu, item-use party menu, choose-half, in-battle party
   menu, and return-from-Summary paths. Field, Summary return, and in-battle
   single-party routes are confirmed in `test_plan.md`; choose-half remains a
   manual target.
...
then aligns both columns onto the same three row baselines for the local
`2 / 2 / 2` screen. It builds a `14 x 5` framed slot from existing party menu
background tiles.
...
- Should choose-half use the same `2 x 3` draw in every facility route, or keep
  any facility-specific Confirm / Cancel treatment?
- Should double / multi battle-order layouts keep the current left/right emphasis
  for active battlers, or eventually also use a grid-family layout?
...
- in-battle single-party menu draws the same grid and still maps to
  `gPlayerParty`; double / multi battle-order layouts remain separate;
