...
| NPC cleanup / replacement | field NPC は map object、hide flag、defeated flag、story gate、movement script、sprite resource と結びつく。Team Aqua / Team Magma 系は story 依存を確認してから触る。 | `docs/flows/npc_object_event_flow_v15.md` / `docs/flows/map_script_flag_var_flow_v15.md` |
| NPC image / object capacity | 既存 sprite を流用するか、外部 NPC art を入れるか、palette / object event 枠をどこまで確保するかは partygen とは別の resource slice。 | `docs/overview/roguelike_npc_capacity_v15.md` |
