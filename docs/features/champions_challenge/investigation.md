...
| Frontier / Pyramid save contract | `src/save.c` `TrySavingData(SAVE_LINK)`, `src/frontier_util.c` `SaveGameFrontier`, `src/battle_pyramid.c` `SavePyramidChallenge` / `PausePyramidChallenge` | 中断 / 復帰の参考になるが、通常 party / bag snapshot には Champions 専用 state が必要。詳細は `docs/features/champions_challenge/run_session_restore.md`。 |
...
| Run session restore / checkpoint | Battle Pyramid 風の一時レポート、autosave、power-off 復帰、通常 party / bag / PC restore policy を runtime 前に固定する必要がある。 | `docs/features/champions_challenge/run_session_restore.md` |
