...
- Battle Pyramid / Frontier の「一時レポート、途中復帰、Frontier-style partial save」の考え方。
...
## Run Session / Restore Position

`docs/features/champions_challenge/run_session_restore.md` で、Battle Pyramid /
Battle Frontier 型の中断 / 復帰 / 退避 / 復元 contract を追加調査した。

結論:

- `SavePlayerParty()` は通常 party の backup ではなく `SaveBlock1.playerParty[]`
  そのものを書き換えるため、0 匹開始の run snapshot には使えない。
- `TrySavingData(SAVE_LINK)` は PC storage を書かないため、checkpoint save の
  参考になる。ただし Champions の run party / run bag / seed / stage / reward
  state までは持たない。
- PC box を申込時点へ完全 rollback するには通常 save layout に full PC
...
partygen / catalog が既存 trainer block を置き換えていても、challenge
runtime が無い間は C 側から `partygen_owned` tag を直接見ることはできない。

後続 runtime 実装では、他 feature が trainer ID 固定リストや長い条件式を
持たなくて済むよう、最小の explicit helper を用意する。
...
tools/champions_partygen/partygen.sh plan --catalog tools/champions_partygen/catalog/journey.json
tools/champions_partygen/partygen.sh generate --seed 1234 --out /tmp/champions_trainers.party
tools/champions_partygen/partygen.sh validate --strict --input /tmp/champions_trainers.party
