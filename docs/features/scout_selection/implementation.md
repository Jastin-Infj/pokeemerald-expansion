...
| 2026-05-20 | Cursor responsiveness pass | Pass | D-pad movement now uses `JOY_REPEAT` while the Scout screen is active, with Scout-local repeat delays of 10 frames initial / 3 frames continued. Same-scroll cursor movement redraws only the previous/current card rows instead of copying the whole list and help windows. mGBA Live `scout-selection-responsive-20260520b` ran at 60 fps target and verified hold-DOWN repeat movement, scroll-boundary redraw, A selection marker redraw, Summary open/return, clean stop, and empty CLI `status --all`. The first fixed-binary mGBA attempt `scout-selection-responsive-20260520` failed before boot because DISPLAY was unset; the wrapper retry passed. |
| 2026-05-20 | Cursor responsiveness local checks | Pass | `rtk git diff --check`, `rtk make -j16 -O debug`, `rtk make -j16 -O all`, `rtk make -j16 -O check`, and `rtk mdbook build docs` passed. Build warnings remain the existing RWX linker warning and mdBook's existing missing root `CHANGELOG.md` include, `CREDITS.md` `</img>`, and large search index warnings. |
| 2026-05-20 | Cursor repeat tuning | Pass | Scout-local D-pad repeat was softened from 10/3 frames to 16-frame initial delay / 5-frame continued delay. This keeps hold-to-scroll responsive while reducing accidental two-step cursor movement when the player only wants to move one slot. |
| 2026-05-20 | Cursor repeat tuning local checks | Pass | `rtk git diff --check`, `rtk make -j16 -O debug`, `rtk make -j16 -O all`, `rtk make -j16 -O check`, and `rtk mdbook build docs` passed with only existing warnings. |
