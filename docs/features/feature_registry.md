...
| Friendly Shop Pokemon Vendor | Planned | Docs-only investigation; no runtime code on `master` | `docs/features/friendly_shop_pokemon_vendor/` | Shop-like Pokemon / Egg products with repeatable or one-time purchase, party-slot Egg risk, optional abstract Egg progress / reward currency, and future edit-entitlement policy. BP is an idea, not a dependency. Implementation should add a dedicated product table / delivery finalizer instead of overloading normal `pokemart` item ids. |
