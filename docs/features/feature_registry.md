...
| Scout Selection / Starting Battlefield Status | Planned | Docs-only feature kickoff; runtime not on `master` | `docs/features/scout_selection/`, `docs/overview/scout_selection_and_battlefield_status_v15.md` | Pokemon Champions 風の候補 Pokemon 選択 UI。Team Viewer の icon grid / Summary return と Battle Factory の 6 候補 selection を参照し、新規 `scout_selection` module として実装する方針。MVP は script-driven pool、最大 6 visible、scroll、Summary 確認、1/N picks、gift path 付与。 |
