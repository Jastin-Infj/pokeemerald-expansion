...
| All Ability Slots Runtime | Validated implementation shelf | Runtime branch `feature/all-ability-slots-runtime-20260523` from `master` `0407f6daf7`; guarded by `B_ALL_ABILITY_SLOTS`; branch config currently defaults `TRUE`; source remains off `master` | `docs/features/all_ability_slots/` | Battle-only all-active ability rule where species slots 0, 1, and hidden slot can apply simultaneously while `abilityNum` remains a representative operation slot. Duplicate abilities dedupe. Ability-changing effects mutate one corresponding slot only, not the whole active set. Summary shows a selected active slot label plus ability name and uses `L` / `R` to cycle descriptions when `P_SUMMARY_SCREEN_ALL_ABILITY_SLOT_SWITCH` is enabled. Ability Capsule / Patch fail in all-slot mode, common direct battle predicates were converted, and focused All Ability Slots coverage is now 50 cases. Full `check` is not a green gate under global `TRUE` because upstream tests assert single-ability defaults. Current open dependency risks are #47 battle item restore, #48 held item tokens, #51 scout selection, #54 party/status UI, and #57 friendly shop Pokemon vendor. See `handoff.md` for the docs-only master handoff. |
