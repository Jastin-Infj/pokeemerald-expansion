...
| Friendly Shop Pokemon Vendor | Implementation started | First runtime slice implements the fixed-species / global no-evolution rule on a feature branch. Vendor menu, sealed recruit delivery, bond / seal EXP, and edit entitlement are still pending. | `docs/features/friendly_shop_pokemon_vendor/` | Shop-like Pokemon / Egg-like sealed recruit products with repeatable or one-time purchase, locked shop rows, party-slot sealed risk, Shadow-like bond / seal EXP release, and future edit-entitlement policy. Final evolutions and legendaries may be allowed by product table. BP is an idea, not a dependency. Implementation should add a dedicated product table / delivery finalizer instead of overloading normal `pokemart` item ids. |
