...
| Battle Item Restore Policy | Implementing | Berry-inclusive restore behind config | `docs/features/battle_item_restore_policy/` | `B_RESTORE_HELD_BATTLE_BERRIES` を追加し、`TryRestoreHeldItems()` で original held item のきのみ復元を有効化。戦闘中の `usedHeldItem` / Recycle / Harvest 等は維持。 |
