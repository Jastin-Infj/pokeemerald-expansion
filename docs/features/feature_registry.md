...
| Champions-style EV/IV Training UI | Investigating | No code changes | `docs/overview/champions_training_ui_feasibility_v15.md` | EV/IV 配分 UI は実装可能。script 単体ではなく C 側 UI を `special` + `waitstate` で起動する方針。EV/IV setter は UI 側 validation 必須。 |
