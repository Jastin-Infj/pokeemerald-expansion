...
| All Ability Slots Runtime | Implementing | Runtime branch `feature/all-ability-slots-runtime-20260523` from `master` `0407f6daf7`; guarded by `B_ALL_ABILITY_SLOTS`; local `all`, `debug`, full `check`, and focused ability checks pass | `docs/features/all_ability_slots/` | Battle-only all-active ability rule where species slots 0, 1, and hidden slot can apply simultaneously while `abilityNum` remains a representative operation slot. Duplicate abilities dedupe. Ability-changing effects mutate one corresponding slot only, not the whole active set. Summary prints compact active ability names, Ability Capsule / Patch fail in all-slot mode, and common direct battle predicates were converted. The ability audit covers remaining representative-only policy areas; focused All Ability Slots coverage is now 49 cases. Related feature dependencies include friendly shop Pokemon vendor #57, party/status UI #54, scout selection #51, held item tokens #48, and battle item restore #47; this dependency scope means branch / PR diff conflicts, not only C call graph edges. AI and polished Summary description browsing remain follow-up risks. |
