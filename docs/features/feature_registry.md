...
| All Ability Slots Runtime | Investigating | Docs-only source dependency audit on `master` `de310ef9eb`; no runtime source changes | `docs/features/all_ability_slots/` | Proposed battle-only all-active ability rule where species slots 0, 1, and hidden slot all apply simultaneously while `abilityNum` remains a representative slot. Duplicate abilities dedupe. Ability-changing effects should mutate one corresponding slot only, not the whole active set. Mega Evolution now uses the Mega species' three current-form slots rather than base-plus-Mega overlay. Current engine is singular around `BattlePokemon.ability`, `GetBattlerAbilityInternal`, `AbilityBattleEffects`, AI caches, Summary, Ability Capsule / Patch, and form-change helpers. Balance tuning should be a separate feature. |
