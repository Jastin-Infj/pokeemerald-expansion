...
| All Ability Slots Runtime | Investigating | Docs-only source dependency audit on `master` `de310ef9eb`; no runtime source changes | `docs/features/all_ability_slots/` | Proposed all-active ability rule where species slots 0, 1, and hidden slot all apply simultaneously. Current engine is singular around `BattlePokemon.ability`, `GetBattlerAbilityInternal`, `AbilityBattleEffects`, AI caches, Summary, Ability Capsule / Patch, and form-change helpers. Save layout can likely remain compatible by leaving `abilityNum` as representative slot and adding set-aware helpers on a fresh runtime branch. |
