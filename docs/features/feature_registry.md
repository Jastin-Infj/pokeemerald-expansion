...
| Provenance | Local project overlay, `gh pr list --state all`, fetched PR refs, branch merge-base diffs, 2026-05-17 PR cleanup, 2026-05-18 Team Viewer / partygen source audit, 2026-05-18 comprehensive inventory, 2026-05-23 all-ability-slots investigation |
...
|---|---|---|
| No Random Encounters | Completed implementation shelf #41 on `feature/no-random-encounters-step-only-runtime-20260517`; closed 2026-05-17 after CI success. User-confirmed Route 101 behavior; docs handoff merged through #42. Adopted into `integration/runtime-dev-20260529` on 2026-05-30. | Runtime behavior is done and staged in PR #68. `master` still has `OW_FLAG_NO_ENCOUNTER 0`; keep source off `master` until runtime policy changes. |
| TM Shop Migration | Completed implementation shelf #31 on `feature/tm-shop-migration`; closed 2026-05-17 after CI success. Docs handoff was merged through PR #30. Adopted into `integration/runtime-dev-20260529` on 2026-05-30. | Runtime behavior is staged in PR #68. `master` remains docs / Lua-only; FRLG legacy acquisition remains follow-up scope. |
| Unified Move Relearner | Completed implementation shelf #28 on `feature/unified-move-relearner`; closed 2026-05-17 after CI success. Docs-only handoff was merged through PR #29. Adopted into `integration/runtime-dev-20260529` on 2026-05-30 after TM Shop Migration. | Runtime behavior is staged in PR #68 with Summary-first route and direct party `RELEARN` disabled by config. `master` remains docs / Lua-only. |
| Summary Tera Type Icon | Completed implementation shelf #26 on `feature/summary-tera-type-badge`; closed 2026-05-17 after CI success. Adopted into `integration/runtime-dev-20260529` on 2026-05-30 after Pokemon State Editor. | Runtime behavior and imported graphics are staged in PR #68. Integration confirmed the Info-page badge and Skills-page State Editor prompt coexist. `master` remains docs / Lua-only. |
| Pokemon State Editor | Completed implementation shelf #23 on `feature/pokemon-state-editor-expansion`; closed 2026-05-17 after CI success. Adopted into `integration/runtime-dev-20260529` on 2026-05-30 after Team Viewer phase2. | Runtime behavior is staged in PR #68. Integration preserved Summary-first ownership, the Unified Move Relearner prompt area, and existing Party UI / vendor / all-ability hooks. `master` remains docs / Lua-only. |
| Pre-Battle / In-Battle Team Viewer | Completed implementation shelf #20 on `feature/prebattle-team-viewer`; phase2 continuation on `feature/prebattle-team-viewer-phase2`; adopted into `integration/runtime-dev-20260529` on 2026-05-30. Source audit confirms pool/randomizer cache behavior is implemented. | Runtime behavior is staged in PR #68. Integration used Team Viewer phase2 as the normal-trainer selection surface, preserved the battle-selection compression/restore helper, and passed `all`, `debug`, full `check`, and mGBA viewer -> Summary -> selection -> battle -> in-battle viewer smoke. `master` remains docs / Lua-only. |
| Trainer Battle Aftercare / Battle Item Restore | `feature/battle-item-restore-policy` の berry-inclusive held item restore は #47 として採用済み。`feature/trainer-battle-aftercare-heal` の aftercare heal hook も 2026-05-30 に `integration/runtime-dev-20260529` へ採用済み。2026-05-31 follow-up で runtime lane は `B_TRAINER_BATTLE_AFTERCARE TRUE` に変更。`master` には `B_TRAINER_BATTLE_AFTERCARE` / `B_RESTORE_HELD_BATTLE_BERRIES` が無い。 | Runtime behavior is staged in PR #68. Config-on win/exclusion validation is now the acceptance gate. `master` remains docs / Lua-only. |
...
| Champions Partygen | `feature/trainer-partygen-catalog-expansion` に Rust CLI、catalog、Elite Four / Wallace Trainer Party Pool data diff、mGBA ROM-memory evidence がある。`integration/runtime-dev-20260529` には 2026-05-30 に catalog / generated trainer data を採用済み。`master` には `tools/champions_partygen/README.md` だけがある。 | runtime integration 側の data shelf として扱う。今後は generated drift / pool balance を追加 review する。Closed #5/#7 は未実装ではなく実装棚。 |
...
| Field Move Modernization / HM Removal | `feature/field-move-modernization-mvp` and `feature/field-move-toolkit-item` hold runtime slices. Docs say both the HM-free MVP and Field Kit itemization were locally validated and user-confirmed. `integration/runtime-dev-20260529` adopted the modernization, Field Kit unlock item, debug scripts, and utility menu on 2026-05-30. | Do not describe this as unimplemented / no-code. Runtime source still stays off `master`; current runtime owner is the integration branch. |
...
| Battle BGM Selector / Sound Archive | Adopted into runtime integration | Runtime source plus BW/BW2, DPPt, Platinum, and HGSS battle BGM imports re-applied to `integration/runtime-dev-20260529`; no code changes on `master` | `docs/features/battle_bgm_selector/` | `feature/battle-bgm-selector-mvp-20260517`; `integration/runtime-dev-20260529` | Debug Trainer/Wild selector plus selected Modern Emerald import slices, including Galactic/Rocket, Platinum, HGSS Kanto, beast, Frontier, and legendary follow-up choices. 2026-05-31 follow-up persists the two choices in existing SaveBlock2 filler with a magic marker. DPPt / Platinum / HGSS uses imported `.aif` sample tooling. Integration fixes the reviewed odd-length `aif2pcm --compress` half-byte issue. Modern Emerald license / permission status remains a master-adoption risk. |
...
| #10 `Add trainer battle aftercare heal hook` | `feature/trainer-battle-aftercare-heal` | Closed 2026-05-10, unmerged. Runtime hook adopted into `integration/runtime-dev-20260529` on 2026-05-30 and enabled on 2026-05-31. | Implemented closed shelf: trainer win heal hook. Config-on normal-win and exclusion-path validation remains required. |
...
| Field Move Modernization / HM Removal | Adopted in runtime integration | Implemented on `feature/field-move-modernization-mvp` and `feature/field-move-toolkit-item`; staged on `integration/runtime-dev-20260529`; not on `master` | `docs/features/field_move_modernization/` | README records the HM-free field move MVP, Field Kit itemization, menu entry, icon/palette handoff, and manual validation as implemented / user-confirmed. Integration added the Field Kit item / debug route stack and repaired HM flag constants. |
...
| Scout Selection / Starting Battlefield Status | Adopted in runtime integration | Runtime MVP on `integration/runtime-dev-20260529`; source shelf PR #51 | `docs/features/scout_selection/`, `docs/overview/scout_selection_and_battlefield_status_v15.md` | Pokemon Champions 風の候補 Pokemon 選択 UI。New `scout_selection` module implements script-driven pool, 12 candidate debug pool, 6 visible slots, scroll, Summary preview/return, 1/N picks, and gift path handoff. The dummy starter pool has been replaced by a generated partygen JSON pool with species de-duplication. 2026-05-29 integration validation confirmed debug route open, Metang Summary return, one-pick gift, and 2x3 Party menu display after gift. |
| Friendly Shop Pokemon Vendor | Adopted in runtime integration | Runtime slice on `integration/runtime-dev-20260529`; source shelf PR #57 / `feature/global-no-evolution-20260523`. Vendor menu, dedicated `pokemonvendor` script command, normal / sealed recruit delivery, PC fallback for full-party sealed purchases, one-time flags, locked rows, bond / seal EXP helpers, actual-species locked display in Summary / party / PC icons, edit-entitlement helper, global no-evolution rule, and UI repair are implemented and locally validated. | `docs/features/friendly_shop_pokemon_vendor/` | Shop-like Pokemon / Egg-like sealed recruit products with repeatable or one-time purchase, locked shop rows, party-carried sealed risk, Shadow-like bond / seal EXP release, and future editor / relearner policy. Final evolutions and legendaries may be allowed by product table. 2026-05-29 integration validation confirmed debug `Script 1` vendor open, Pikachu prompt, and purchase success after #47/#48/#54/#51. BP remains an idea, not a dependency. |
| All Ability Slots Runtime | Adopted in runtime integration | Runtime source on `integration/runtime-dev-20260529`; source shelf PR #60 / `feature/all-ability-slots-runtime-20260523`; integration default switched to `B_ALL_ABILITY_SLOTS TRUE` on 2026-05-31 so normal battles use all active slots; source remains off `master` | `docs/features/all_ability_slots/` | Battle-only all-active ability rule where species slots 0, 1, and hidden slot can apply simultaneously while `abilityNum` remains a representative operation slot. Duplicate abilities dedupe. Ability-changing effects mutate one corresponding slot only, not the whole active set. Summary shows a selected active slot label plus ability name and uses `L` / `R` to cycle descriptions when `P_SUMMARY_SCREEN_ALL_ABILITY_SLOT_SWITCH` is enabled. Ability Capsule / Patch fail in all-slot mode, common direct battle predicates were converted, and focused All Ability Slots coverage is now 50 cases. 2026-05-29 integration validation passed focused All Ability, AI thinking-time, item restore, vendor, `all`, `debug`, full `check`, and mGBA Live Pattern A recoil smoke after #47/#48/#54/#51/#57. 2026-05-31 default-TRUE follow-up passes focused All Ability, `all`, `debug`, docs, and mGBA boot; full `check` is expected-red under global TRUE because upstream tests still assert single-ability defaults. |
| Champions Challenge Facility | Adopted run-session MVP in runtime integration | Run session restore source on `integration/runtime-dev-20260529`; source shelf PR #62 / `feature/champions-run-session-runtime-20260524`; partygen implementation still separate on `feature/trainer-partygen-catalog-expansion`; runtime source remains off `master` | `docs/features/champions_challenge/` | 0 匹開始、6 匹作成、Lv.50 battle-only、EXP 無効、bag 退避 / 空 challenge bag、egg-only default eligibility、optional Frontier ban、敗北時 challenge party 破棄と通常 party / bag 復元の仕様を整理。Run session restore MVP adds `SaveBlock1.championsRun`, active-run temporary report routing, normal party / bag / money snapshot restore, PC block while active, EXP / Bag / held-item restrictions, clear carryover config, and named debug routes. 2026-05-29 integration validation passed Champions, SaveBlock, All Ability, item restore, vendor, `all`, `debug`, full `check`, and mGBA Start / Give Mon / Retire smoke after #47/#48/#54/#51/#57/#60. |
| Party / Status UI Overhaul | Adopted in runtime integration | Runtime source on `integration/runtime-dev-20260529`; source shelf PR #54 | `docs/features/party_status_ui_overhaul/` | Party screen を現行 `1 + 5` から `2 columns x 3 rows` (`2 / 2 / 2`) へ寄せる候補。#54 の source slice は #47/#48 後の runtime integration branch に採用済み。画像 assets は docs-only master branch へ入れず、runtime branch 側で code と一緒に扱う。手動 UI 確認は Party / Switch / Summary return / held-item Give-Take を #48 active 状態で再確認する。 |
...
| Trainer Battle Aftercare / Forced Release | Adopted heal-only hook in runtime integration | Default-off heal hook is staged on `integration/runtime-dev-20260529`; not on `master` | `docs/features/trainer_battle_aftercare/` | `feature/trainer-battle-aftercare-heal` 由来の `B_TRAINER_BATTLE_AFTERCARE` default off hook を採用済み。no-whiteout、forced release、battle selection integration は後続。 |
