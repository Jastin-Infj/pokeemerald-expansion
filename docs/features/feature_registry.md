...
| Trainer Battle Aftercare / Battle Item Restore | `feature/battle-item-restore-policy` の berry-inclusive held item restore は #47 として採用済み。`feature/trainer-battle-aftercare-heal` の default-off aftercare heal hook も 2026-05-30 に `integration/runtime-dev-20260529` へ採用済み。`master` には `B_TRAINER_BATTLE_AFTERCARE` / `B_RESTORE_HELD_BATTLE_BERRIES` が無い。 | Runtime behavior is staged in PR #68. Aftercare remains opt-in (`FALSE`) and needs config-on win/exclusion validation before default enable. `master` remains docs / Lua-only. |
...
| #10 `Add trainer battle aftercare heal hook` | `feature/trainer-battle-aftercare-heal` | Closed 2026-05-10, unmerged. Runtime hook adopted into `integration/runtime-dev-20260529` on 2026-05-30. | Implemented closed shelf: default-off trainer win heal hook. Config-on normal-win and exclusion-path validation remains before enabling by default. |
