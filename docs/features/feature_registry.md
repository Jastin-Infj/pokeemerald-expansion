...
| Friendly Shop Pokemon Vendor | Implemented on feature branch | Runtime slice on `feature/global-no-evolution-20260523`; not on `master`. Vendor menu, dedicated `pokemonvendor` script command, normal / sealed recruit delivery, PC fallback for full-party sealed purchases, one-time flags, locked rows, bond / seal EXP helpers, actual-species locked display in Summary / party / PC icons, edit-entitlement helper, global no-evolution rule, and UI repair are implemented and locally validated. | `docs/features/friendly_shop_pokemon_vendor/` | Shop-like Pokemon / Egg-like sealed recruit products with repeatable or one-time purchase, locked shop rows, party-carried sealed risk, Shadow-like bond / seal EXP release, and future editor / relearner policy. Final evolutions and legendaries may be allowed by product table. BP remains an idea, not a dependency. |
