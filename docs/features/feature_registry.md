...
| Battle Item Restore Policy | Integration candidate | Current-master feature branch implemented; not on `master` | `docs/features/battle_item_restore_policy/` | #47 / `feature/battle-item-restore-current-master-20260519` に `B_RESTORE_HELD_BATTLE_BERRIES` default `TRUE`、`TryRestoreHeldItems()` の berry restore、direct / full battle tests、full `check`、mGBA Live boot/input evidence がある。旧 `feature/battle-item-restore-policy` / #14 は historical shelf。`master` へは source 未反映。 |
