...
| Party / Status UI Overhaul | Investigating | No code changes | `docs/features/party_status_ui_overhaul/` | Party screen を現行 `1 + 5` から `2 columns x 3 rows` (`2 / 2 / 2`) へ寄せる候補。実装には新しい party background / slot frame / tilemap 画像が必要。BW Summary / DS-style party screen は RavePossum Emerald Extra と Xaman credit line を外部参考として登録済み。画像 assets は docs-only master branch へ入れず、credit 確認後に runtime implementation branch で code と一緒に取り込む。UI 自体は SaveBlock 非消費を原則にし、保存 option 化する場合だけ SaveBlock2 option policy を使う。 |
