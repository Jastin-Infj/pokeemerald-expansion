...
| 5 | `docs/features/battle_item_restore_policy/` | Shipped on feature branch | battle 中の消費処理を変えず、battle-end restore だけを policy 化する。berry-heavy partygen pool への影響を guard で分ける | aftercare guard contract |
...
| Battle Item Restore Policy | Shipped | Berry-inclusive restore behind config, user-confirmed in game | `docs/features/battle_item_restore_policy/` | `B_RESTORE_HELD_BATTLE_BERRIES` を追加し、`TryRestoreHeldItems()` で original held item のきのみ復元を有効化。戦闘中の `usedHeldItem` / Recycle / Harvest 等は維持。実装記録は `implementation.md`。 |
| Trainer Battle Aftercare / Forced Release | Implementing | Heal-only MVP behind config | `docs/features/trainer_battle_aftercare/` | `B_TRAINER_BATTLE_AFTERCARE` default off。通常 trainer battle 勝利後の heal-only hook を `CB2_EndTrainerBattle` に追加。no-whiteout、forced release、battle selection integration は後続。 |
