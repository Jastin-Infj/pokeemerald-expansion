...
| Form-specific special inclusion | Use form species with form-only moves, such as Wash Rotom or Pikachu Libre. | Appliance / costume moves appear as special candidates for the matching form only. |
| LGPE partner special inclusion | Use `SPECIES_PIKACHU_STARTER` and `SPECIES_EEVEE_STARTER`. | Partner-exclusive tutor and Partner Power moves appear as special candidates. |
...
| 2026-05-16 | Expanded special JSON smoke | Pass | Runtime JSON validates as 178 candidate blocks / 229 moves with no unknown species or move constants. mGBA Live created Bulbasaur through debug `Give X -> Pokemon (Basic)` and confirmed `Celebrate` as a new `Sp` row in party `RELEARN`. Screenshot: `/tmp/unified_move_relearner_special_expanded_bulbasaur_after_hold_right4.png`. Cleanup returned `mgba-live-cli status --all` to `[]`. |
| 2026-05-16 | Supplemental form JSON audit | Pass | Runtime JSON has 32 source refs, 178 candidate blocks, and 229 moves with no unknown species or move constants. Supplemental emitted species include Rotom appliance forms, Cosplay Pikachu forms, LGPE Partner / Starter Pikachu and Eevee, Kyurem White/Black, and Oinkologne F. |
| 2026-05-16 | mGBA Live Bellossom Moonblast check | Pass | Debug-created Lv1 Bellossom already knew `Moonblast`; party `RELEARN` hid it as an already-known move. Screenshots: `/tmp/unified_move_relearner_bellossom_list_no_known_moonblast.png`, `/tmp/unified_move_relearner_bellossom_moonblast_known_summary.png`. |
| 2026-05-16 | mGBA Live Rotom form special | Pass | Debug-created Wash Rotom, opened party `RELEARN`, page-scrolled to `Hydro Pump` as an `Sp` row. Screenshot: `/tmp/unified_move_relearner_rotom_wash_hydro_pump_sp.png`. |
| 2026-05-16 | mGBA Live Cosplay Pikachu special | Pass | Debug-created Pikachu Libre, opened party `RELEARN`, page-scrolled to `Flying Press` as an `Sp` row. Screenshot: `/tmp/unified_move_relearner_pikachu_libre_flying_press_sp.png`. |
