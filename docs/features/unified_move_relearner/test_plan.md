...
| 2026-05-16 | Expanded special JSON smoke | Pass | Runtime JSON validates as 174 candidate blocks / 216 moves with no unknown species or move constants. mGBA Live created Bulbasaur through debug `Give X -> Pokemon (Basic)` and confirmed `Celebrate` as a new `Sp` row in party `RELEARN`. Screenshot: `/tmp/unified_move_relearner_special_expanded_bulbasaur_after_hold_right4.png`. Cleanup returned `mgba-live-cli status --all` to `[]`. |
| 2026-05-16 | Supplemental form JSON audit | Pass | Runtime JSON has 31 source refs, 174 candidate blocks, and 216 moves with no unknown species or move constants. Supplemental emitted species include Rotom appliance forms, Cosplay Pikachu forms, Kyurem White/Black, and Oinkologne F. |
