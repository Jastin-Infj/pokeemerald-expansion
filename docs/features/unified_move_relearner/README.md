...
| Code status | Planned; docs-only scope refresh |
| Provenance | User request and local code/docs review |

## Status

Status: Investigating / planned. No runtime source changes are part of this
docs refresh.
...
| move source display | 重複を許容するため、`Lv` / `Egg` / `TM` / `Tutor` / `Tower` などの source 表示が実質必須。 |
