...
- repo の species 名は `all_teaching_types.json` に出てくるものを基準にし、
  matching できない source entry は runtime table に出さない。
...
| porymoves union species keys | 1114 |
| repo teaching species matched in porymoves | 1100 |
| repo teaching species missing from porymoves | 0 |
...
については porymoves 側と matching できている。一方で、porymoves 側にある
form-specific key が repo の teaching species 名と 1:1 で一致しない場合、その
form-specific historical candidate は今の generator では使われない。Rotom form や
Kyurem form のような form-specific source data は、必要なら次の data audit で明示的な
alias map を足す。
