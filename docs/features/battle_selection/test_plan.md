...
Validated for normal trainer battle MVP. Build / check / mGBA boot smoke passed on
2026-05-09. User manual validation confirmed single and double trainer battle
selection, battle start, and party restore.

## Validation Results

| Date | Command / Check | Result | Notes |
|---|---|---|---|
| 2026-05-09 | `rtk git diff --check` | Pass | whitespace check。 |
| 2026-05-09 | `rtk make -j16 -O all` | Pass | `B_TRAINER_BATTLE_SELECTION=TRUE` で通常 ROM build。linker RWX warning は既存。 |
...
2026-05-09 の user manual validation で、通常 single / double trainer battle の
core flow は確認済み。exclusion flow は未確認のため、後続 regression で見る。
2026-05-31 の user manual validation で、runtime integration branch の short-party
selection behavior も確認済み。
...
- cancel は MVP では無効。将来 cancel を許す仕様にする場合、script 復帰先と manual expected result を更新する必要がある。
...
- battle transition animation 中の player / NPC trainer sprite の影表示を polish するかは未決定。
