...
| short eligible party | `B_TRAINER_BATTLE_SELECTION_ALLOW_SHORT_PARTY == TRUE` なら、eligible party が通常要求数 3/4 未満でも、single は `B_TRAINER_BATTLE_SELECTION_SHORT_PARTY_MIN_COUNT` 以上、double は `B_TRAINER_BATTLE_SELECTION_SHORT_DOUBLE_MIN_COUNT` 以上なら実際の eligible 数を選出数として UI を開く。デフォルトでは single 1 匹から、double 2 匹から許可する。 |
