...
MVP は既存 flag を使う。新しい encounter table や randomizer と混ぜない。`feature/no-random-encounters-step-only-runtime-20260517` では `FLAG_UNUSED_0x8E5` を `FLAG_NO_ENCOUNTER` に rename し、`OW_FLAG_NO_ENCOUNTER` へ割り当てる 3 file 実装を current `master` から切り直して再検証済み。

## 2026-05-17 Runtime Adoption Notes

Earlier 2026-05-17 docs-only review kept source / include off `master`. The
runtime branch now reapplies the slice from current `master` as a fresh branch
instead of merging an older branch. The intended runtime diff remains these 3
files only:
...
`master` and re-apply only the three runtime files:

- `include/config/overworld.h`
- `include/constants/flags.h`
- `include/constants/flags_frlg.h`

Do not merge the old `feature/no-random-encounters` branch into `master`; it was
created before later docs updates and can roll back current documentation if
merged directly.
...
- `docs/features/no_random_encounters/implementation.md`
...
- `docs/manuals/github_workflow.md`
