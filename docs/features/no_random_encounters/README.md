...
| Baseline | `master` `050a5ab7a3`; branch implementation exists on `feature/no-random-encounters` |
...
`feature/no-random-encounters`, but source changes are intentionally not copied
to `master`. `master` remains the upstream intake baseline plus docs / workflow
overlay. Keep `include/config/overworld.h` at `OW_FLAG_NO_ENCOUNTER 0` on
`master`.
...
Do not merge the old feature branch into `master`; it was created before later
docs updates and can roll back current documentation if merged directly.
