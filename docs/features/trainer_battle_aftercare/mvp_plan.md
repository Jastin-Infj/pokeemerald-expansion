...
上記の heal-only helper を実装済み。将来 Champions Challenge runtime が入ったら、
...
- `B_TRAINER_BATTLE_AFTERCARE` config, default `FALSE`.
- `TrainerBattleAftercare_ShouldApply()` guard helper.
- `TrainerBattleAftercare_ApplyIfEnabled()` heal-only helper.
- Normal trainer win heal only when the config is enabled.

Still planned:

- no-whiteout trainer loss policy.
- forced release helper / post-battle state machine.
- battle selection restore integration.
- Champions Challenge runtime bypass via `ChampionsChallenge_IsActive()`.
