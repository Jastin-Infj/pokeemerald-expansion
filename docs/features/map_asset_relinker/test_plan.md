...
| GUI Playwright dry-run execution | With the same Route301 setup, click `Run Dry-Run` | The GUI status becomes `Dry-run complete`, displays a `/tmp/route401_relink_*.json` plan path, and shows the existing CLI dry-run output: layout/map moves, structured edits, review references, and `Dry-run complete; no files changed.` |
