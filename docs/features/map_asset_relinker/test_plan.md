...
| GUI project root picker | Packaged Tauri app startup, or `Open...` in the top bar | The desktop app opens a native directory picker. Selecting a pokeemerald-expansion root fills the Project Root field and immediately scans. Cancelling leaves the app idle with `Choose a project root to scan`; `Scan` does not guess a root when the packaged app has none. |
...
| GUI Windows artifact workflow | `.github/workflows/map-asset-relinker-desktop.yml` on `windows-latest` | Runs the Windows build helper and uploads `map-asset-relinker-windows`, containing the CUI `.exe`, direct GUI `.exe`, setup `.exe`, and `.msi` bundle. Run `26455505119` passed on the current branch head and uploaded artifact id `7217867302`. |
...
- `TAURI_LOCAL_DEPS=/tmp/tauri-linux-deps
  ...
  Tauri dialog plugin, confirming the packaged Linux build still works with the
  native project-root picker dependency.
...
- `tools/map_asset_relinker_gui/scripts/build_windows.ps1` was updated to
  ...
  `tauri.conf.json`, run `26455505119` passed on the current branch head and
  uploaded `map-asset-relinker-windows` artifact id `7217867302`.
- Downloading `map-asset-relinker-windows` to `/tmp/map-asset-relinker-windows`
