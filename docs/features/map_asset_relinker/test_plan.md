...
| GUI Windows artifact workflow | `.github/workflows/map-asset-relinker-desktop.yml` on `windows-latest` | Runs the Windows build helper and uploads `map-asset-relinker-windows`, containing the CUI `.exe`, direct GUI `.exe`, setup `.exe`, and `.msi` bundle. Run `26454582655` passed on the current branch head and uploaded artifact id `7217447692`. |
...
- `tools/map_asset_relinker_gui/scripts/build_windows.ps1` was updated to
  ...
  `tauri.conf.json`, run `26454582655` passed on the current branch head and
  uploaded `map-asset-relinker-windows` artifact id `7217447692`.
