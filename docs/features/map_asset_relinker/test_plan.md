...
| GUI project root picker | Packaged Tauri app startup, then `Explorer...` in the top bar / empty state | Startup does not auto-open Explorer and should remain responsive at `Choose a project root to scan`. Pressing `Explorer...` opens a native directory picker through the non-blocking Rust/Tauri backend command. Selecting a pokeemerald-expansion root fills the Project Root field and immediately scans. Cancelling leaves the app idle; `Scan` does not guess a root when the packaged app has none. |
| GUI diagnostics log | Packaged Tauri app startup; click `Diagnostics` | The app writes JSONL events for app start, folder-picker request/result/error, scan start/result/error, and diagnostics access. The panel shows the log path and recent lines. On Windows the expected path is `%APPDATA%\Map Asset Relinker\logs\diagnostics.jsonl`. |
...
| Release zip packaging | `.github/workflows/map-asset-relinker-desktop.yml` on branch / PR / tag | Creates `map-asset-relinker-windows-portable.zip` from `dist-windows/portable/` and uploads it as `map-asset-relinker-windows-portable-zip`. A `map-asset-relinker-v*` tag also publishes the zip to the matching GitHub Release. |
| Release workflow review | `codex review --uncommitted` | No actionable defects reported after scoping `contents: write` to the tag-only publish job. Branch / PR builds remain read-only and only upload Actions artifacts. |
| 2026-05-31 integration Windows artifact check | `gh api repos/Jastin-Infj/pokeemerald-expansion/actions/runs/26708238131/artifacts` | Latest checked successful desktop workflow for `integration/runtime-dev-20260529` uploaded `map-asset-relinker-portable-windows` artifact id `7315400434` and `map-asset-relinker-windows` artifact id `7315400291`; both are unexpired. The portable artifact was downloaded locally to ignored `tools/map_asset_relinker_gui/dist-windows/portable/` and contains both `.exe` files plus `README.txt`. |
| 2026-05-31 16.0 replay tool check | `rtk cargo test --manifest-path tools/map_asset_relinker_core/Cargo.toml`; `rtk bash tools/map_asset_relinker/test_map_relink.sh`; `rtk npm run build`; `tools/map_asset_relinker_gui/scripts/build_core_release.sh`; core release `audit --root .`; `TAURI_LOCAL_DEPS=/tmp/tauri-linux-deps tools/map_asset_relinker_gui/scripts/build_native.sh` | Rust core, shell fixture, web build, Linux CUI release binary, audit, and Linux Tauri native build passed on `integration/runtime-dev-16-20260531`. The audit reports 4 existing unused-house warnings across 939 maps. Windows `.exe` remains an Actions / release artifact, not a committed file. |
...
- Latest checked integration artifact evidence: workflow run `26708238131`
  uploaded `map-asset-relinker-portable-windows` artifact id `7315400434` and
  `map-asset-relinker-windows` artifact id `7315400291`, both unexpired as of
  2026-05-31.
- `rtk gh run download 26708238131 -n map-asset-relinker-portable-windows -D
  tools/map_asset_relinker_gui/dist-windows/portable` expanded the portable
  package locally. The ignored handoff folder contains
  `map-asset-relinker-gui.exe`, `map-asset-relinker-core.exe`, and `README.txt`.
...
- `TAURI_LOCAL_DEPS=/tmp/tauri-linux-deps
  tools/map_asset_relinker_gui/scripts/build_native.sh` passes after changing
  the Rust-side native folder picker command to the non-blocking dialog API and
  removing startup auto-open, confirming the packaged Linux build still works
  with native project-root picker support.
...
- Rust core `plan` plus Rust core `apply --dry-run` fixture checks pass for
  the basic `OldCave_2:OldCave_2F` rename/group move and the mixed-case mapsec
  `OldCave_2:Jongle` repair. The dry-run output reports the expected moves,
...
- `tools/map_asset_relinker_gui/scripts/build_windows.ps1` was updated to
  build the Windows direct GUI `.exe` and NSIS setup `.exe`, then copy the
