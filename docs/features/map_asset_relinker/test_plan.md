...
| GUI Windows artifact workflow | `.github/workflows/map-asset-relinker-desktop.yml` on `windows-latest` | Runs the Windows build helper and uploads `map-asset-relinker-windows`, containing the CUI `.exe`, direct GUI `.exe`, setup `.exe`, and `.msi` bundle. Run `26553094568` passed for commit `fedc3c302f` after adding the native project-root picker, diagnostics log, and non-blocking Explorer fix and uploaded artifact id `7257231765`. |
...
- `tools/map_asset_relinker_gui/scripts/build_windows.ps1` was updated to
  ...
  log, run `26553094568` passed for commit `fedc3c302f` and uploaded
  `map-asset-relinker-windows` artifact id `7257231765`.
- Downloading `map-asset-relinker-windows` to
  `/tmp/map-asset-relinker-windows-26553094568`
  ...
  Relinker_0.1.0_x64-setup.exe`,
  `map_asset_relinker_gui/src-tauri/target/release/bundle/msi/Map Asset
  Relinker_0.1.0_x64_en-US.msi`, and
  `map_asset_relinker_core/target/release/map-asset-relinker-core.exe`.
