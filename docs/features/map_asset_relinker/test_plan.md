...
| GUI project root picker | Packaged Tauri app startup, or `Explorer...` in the top bar / empty state | The desktop app opens a native directory picker through the Rust/Tauri backend command first, with the JavaScript dialog plugin as fallback. Selecting a pokeemerald-expansion root fills the Project Root field and immediately scans. Cancelling leaves the app idle with `Choose a project root to scan`; `Scan` does not guess a root when the packaged app has none. |
