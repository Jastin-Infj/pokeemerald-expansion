...
const DEFAULT_BST_BUDGET_TOLERANCE: f64 = 35.0;
...
fn cmd_doctor(opts: &Options) -> Result<i32> {
    ...
    let species = SpeciesIndex::load(repo)?;
    ...
    println!("species info: {}", species.entries.len());
...
fn cmd_generate(opts: &Options) -> Result<i32> {
    ...
    let species = SpeciesIndex::load(&opts.rom_repo)?;
    let mut report = Report::default();
...
fn select_sets<'a>(
    ...
    validate_preferred_maxes(blueprint, &selected)?;
    ...
    for pref in &blueprint.preferred {
        ...
        for _ in 0..needed {
            ...
            let candidates: Vec<&Set> = sets
                ...
                .filter(|set| !used.contains(&set.id))
                .filter(|set| set_allowed_for_blueprint(blueprint, set))
                .filter(|set| set_keeps_preferred_maxes(blueprint, &selected, set))
                .filter(|set| set.roles.contains(&pref.role))
                .collect();
...
fn validate_preferred_maxes(blueprint: &Blueprint, selected: &[&Set]) -> Result<()> {
    for pref in &blueprint.preferred {
        if pref.max == usize::MAX {
            continue;
        }
        let count = selected
            .iter()
        ...
        if count > pref.max {
            return Err(format!(
                "{} required slots already exceed preferred max for role {}: {} > {}",
                blueprint.id, pref.role, count, pref.max
            ));
        }
    }
    Ok(())
...
fn set_keeps_preferred_maxes(blueprint: &Blueprint, selected: &[&Set], candidate: &Set) -> bool {
    blueprint.preferred.iter().all(|pref| {
        pref.max == usize::MAX
            || !candidate.roles.contains(&pref.role)
            || selected
                .iter()
                .filter(|set| set.roles.contains(&pref.role))
                .count()
...
fn run_trainer_lints(
    ...
    run_bst_budget_lint(blueprint, source, sets, species, out);
    run_power_rank_lints(blueprint, source, sets, species, out);
}

fn run_bst_budget_lint(
    blueprint: &Blueprint,
    source: &PartyBlock,
    sets: &[&Set],
    species: &SpeciesIndex,
    ...
    let Some(budget) = blueprint.bst_budget else {
        return;
    };
    if sets.is_empty() {
        return;
    ...
    for set in sets {
        match bst_for_set(set, species) {
            Some(bst) => total += bst,
            None => missing.push(set.id.as_str()),
        }
    }
    ...
    if !missing.is_empty() {
        out.push(LintIssue::note(
            ...
            Some(&source.name),
            &format!(
                "bstBudget configured but {} selected set(s) have no bst; skipping budget check: {}",
                missing.len(),
                missing.join(", ")
            ),
        ));
    ...
    let mean = total / sets.len() as f64;
    let min = budget.mean - budget.tolerance;
    let max = budget.mean + budget.tolerance;
    if mean < min || mean > max {
        out.push(LintIssue::error_(
            "RNK002",
            Some(&source.name),
            &format!(
                "pool BST mean {:.1} is outside budget {:.1} +/- {:.1}",
                mean, budget.mean, budget.tolerance
            ),
        ));
    }
}

fn run_power_rank_lints(
    blueprint: &Blueprint,
    source: &PartyBlock,
    sets: &[&Set],
    species: &SpeciesIndex,
    ...
    let rank = normalized_rank(&blueprint.rank);
    if rank == "mid" {
        let high_power: Vec<&str> = sets
            .iter()
            .filter(|set| {
                species
                    .entries
                    .get(&set.species)
                    .map(|meta| {
                        meta.fully_evolved && bst_for_set(set, species).unwrap_or(0.0) >= 600.0
                    })
                    .unwrap_or(false)
            })
        ...
        if high_power.len() > 1 {
            out.push(LintIssue::warning(
                ...
                Some(&source.name),
                &format!(
                    "mid-rank pool contains {} fully evolved BST >= 600 set(s): {}",
                    high_power.len(),
                    high_power.join(", ")
                ),
            ));
        ...
        if let Some(source_top_level) = source_top_level(source) {
            for set in sets {
                if set.level > source_top_level.saturating_add(5) {
                    out.push(LintIssue::warning(
                        "RNK004",
                        Some(&source.name),
                        &format!(
                            "{} level {} is more than +5 over source top level {}",
                            set.id, set.level, source_top_level
                        ),
                    ));
                }
            }
    ...
    if matches!(rank, "late" | "champion") {
        let has_fully_evolved = sets.iter().any(|set| {
            species
                .entries
                .get(&set.species)
                .map(|meta| meta.fully_evolved)
                .unwrap_or(false)
        });
        if !has_fully_evolved {
            out.push(LintIssue::note(
                "RNK005",
                Some(&source.name),
                "late/champion pool contains zero fully evolved species",
            ));
        }
    }
}

fn bst_for_set(set: &Set, species: &SpeciesIndex) -> Option<f64> {
    set.bst.or_else(|| {
        species
            .entries
            .get(&set.species)
            .map(|meta| meta.bst as f64)
    })
}

fn source_top_level(source: &PartyBlock) -> Option<u16> {
    source
        .text
        .lines()
        .filter_map(|line| line.trim().strip_prefix("Level:"))
        .filter_map(|value| parse_leading_u16(value.trim()))
        .max()
}

fn normalized_rank(rank: &str) -> &str {
    rank.strip_prefix("rank.").unwrap_or(rank)
}
...
#[derive(Default)]
struct SpeciesIndex {
    entries: BTreeMap<String, SpeciesMeta>,
}
...
impl SpeciesIndex {
    fn load(repo: &Path) -> Result<Self> {
        let dir = repo.join("src/data/pokemon/species_info");
        let mut entries = BTreeMap::new();
        for file in list_files_with_extension(&dir, "h")? {
            parse_species_info_file(&read_to_string(&file)?, &mut entries);
        }
        if entries.is_empty() {
            return Err(format!(
                "{} produced no species info entries",
                dir.display()
            ));
        }
        Ok(Self { entries })
...
fn parse_species_info_file(text: &str, entries: &mut BTreeMap<String, SpeciesMeta>) {
    let mut current_species: Option<String> = None;
    let mut stats: [Option<u16>; 6] = [None, None, None, None, None, None];
    let mut has_evolutions = false;

    for line in text.lines() {
        let trimmed = line.trim();
        if let Some(species) = parse_species_header(trimmed) {
            finish_species_entry(&mut current_species, &mut stats, has_evolutions, entries);
        ...
        if trimmed.starts_with(".evolutions") {
            has_evolutions = true;
        }
        if let Some(value) = parse_stat_line(trimmed, ".baseHP") {
            stats[0] = Some(value);
        } else if let Some(value) = parse_stat_line(trimmed, ".baseAttack") {
            stats[1] = Some(value);
        } else if let Some(value) = parse_stat_line(trimmed, ".baseDefense") {
            stats[2] = Some(value);
        } else if let Some(value) = parse_stat_line(trimmed, ".baseSpeed") {
            stats[3] = Some(value);
        } else if let Some(value) = parse_stat_line(trimmed, ".baseSpAttack") {
            stats[4] = Some(value);
        } else if let Some(value) = parse_stat_line(trimmed, ".baseSpDefense") {
            stats[5] = Some(value);
        }
    }
...
fn finish_species_entry(
    current_species: &mut Option<String>,
    stats: &mut [Option<u16>; 6],
    has_evolutions: bool,
    entries: &mut BTreeMap<String, SpeciesMeta>,
) {
    let Some(species) = current_species.take() else {
        return;
    };
    if let [Some(hp), Some(atk), Some(def), Some(speed), Some(sp_atk), Some(sp_def)] = *stats {
        entries.insert(
            species,
            SpeciesMeta {
                bst: hp + atk + def + speed + sp_atk + sp_def,
                fully_evolved: !has_evolutions,
            },
        );
    }
...
fn parse_species_header(line: &str) -> Option<String> {
    let rest = line.strip_prefix("[SPECIES_")?;
    let end = rest.find(']')?;
    Some(format!("SPECIES_{}", &rest[..end]))
}

fn parse_stat_line(line: &str, field: &str) -> Option<u16> {
    let rest = line.strip_prefix(field)?;
    let value = rest.split_once('=')?.1.trim().trim_end_matches(',').trim();
    parse_stat_value(value)
}

fn parse_stat_value(value: &str) -> Option<u16> {
    if let Some((_, rest)) = value.split_once('?') {
        let true_branch = rest.split(':').next()?.trim();
        return parse_leading_u16(true_branch);
    }
    parse_leading_u16(value)
}
...
fn list_json_files(path: &Path) -> Result<Vec<PathBuf>> {
    list_files_with_extension(path, "json")
}

fn list_files_with_extension(path: &Path, extension: &str) -> Result<Vec<PathBuf>> {
    if !path.exists() {
        return Ok(Vec::new());
    }
...
fn parse_blueprint(json: &Json) -> Result<Blueprint> {
    ...
    Ok(Blueprint {
        ...
        bst_budget: parse_bst_budget(obj)?,
    })
}

fn parse_bst_budget(obj: &BTreeMap<String, Json>) -> Result<Option<BstBudget>> {
    let Some(value) = obj.get_optional("bstBudget") else {
        return Ok(None);
    };
    ...
    let budget = value
        .obj()
        .map_err(|_| "bstBudget: expected object".to_string())?;
    let mean = budget.get_f64("mean")?;
    let tolerance = budget.get_f64_default("tolerance", DEFAULT_BST_BUDGET_TOLERANCE)?;
    if mean <= 0.0 {
        return Err("bstBudget.mean must be positive".to_string());
    }
    if tolerance < 0.0 {
        return Err("bstBudget.tolerance must be non-negative".to_string());
    }
    Ok(Some(BstBudget { mean, tolerance }))
}
...
trait JsonObjectExt {
    ...
    fn get_optional_f64(&self, key: &str) -> Result<Option<f64>>;
...
mod tests {
    ...
    fn test_source(text: &str) -> PartyBlock {
        PartyBlock {
    ...
    fn test_blueprint(rank: &str) -> Blueprint {
        Blueprint {
            id: "b.test".to_string(),
            party_size: 2,
            pool_size: 2,
    ...
    fn test_set(id: &str, species: &str, level: u16) -> Set {
        Set {
            id: id.to_string(),
    ...
    fn test_species(entries: &[(&str, u16, bool)]) -> SpeciesIndex {
        let mut index = SpeciesIndex::default();
        for (species, bst, fully_evolved) in entries {
    ...
    fn lint_detects_weather_pair_imbalance() {
        ...
        let species = SpeciesIndex::default();
        run_trainer_lints(&blueprint, &source, &[&setter], &cfg, &species, &mut issues);
    ...
    #[test]
    fn selection_respects_preferred_role_max_during_fill() {
        let mut blueprint = test_blueprint("champion");
        blueprint.party_size = 3;
        blueprint.pool_size = 3;
        blueprint.preferred = vec![PreferredRole {
            role: "role.support".to_string(),
            min: 1,
            max: 1,
        }];

        let mut support_a = test_set("s.support_a", "SPECIES_ABRA", 50);
        support_a.roles = vec!["role.support".to_string()];
        let mut support_b = test_set("s.support_b", "SPECIES_KADABRA", 50);
        support_b.roles = vec!["role.support".to_string()];
        let attacker = test_set("s.attacker", "SPECIES_ALAKAZAM", 50);
        let pivot = test_set("s.pivot", "SPECIES_DROWZEE", 50);
        let sets = vec![support_a, support_b, attacker, pivot];
        ...
        let support_count = selection
            .sets
            .iter()
            .filter(|set| set.roles.contains(&"role.support".to_string()))
            .count();
        assert_eq!(support_count, 1);
    }

    #[test]
    fn validate_fragment_rejects_spaced_numbered_tags() {
        let constants = ConstantIndex {
            trainers: BTreeSet::from(["TRAINER_X".to_string()]),
            species: BTreeSet::from(["SPECIES_ABRA".to_string()]),
            moves: BTreeSet::from(["MOVE_TACKLE".to_string()]),
            items: BTreeSet::new(),
            abilities: BTreeSet::new(),
            natures: BTreeSet::new(),
        };
        let fragment = "=== TRAINER_X ===\nName: X\n\nSPECIES_ABRA\nTags: Tag 6\n- MOVE_TACKLE\n";
        let issues = validate_party_fragment(fragment, &constants);
        assert!(issues.iter().any(|issue| issue.id == "FRG004"));
    }

    #[test]
    fn lint_detects_bst_budget_mean_outside_tolerance() {
        let blueprint = Blueprint {
            id: "b.power".to_string(),
            party_size: 2,
            pool_size: 2,
            ruleset_id: "POOL_RULESET_BASIC".to_string(),
            required: vec![],
        ...
        let low = Set {
            id: "s.low".to_string(),
            species: "SPECIES_MIGHTYENA".to_string(),
            ability: None,
            moves: vec![],
            item: None,
            ivs: "31/31/31/31/31/31".to_string(),
        ...
        assert!(issues
            .iter()
            .any(|i| i.id == "RNK002" && matches!(i.severity, Severity::Error)));
    }

    #[test]
    fn parses_species_info_bst_and_evolution_state() {
        let text = r#"
            [SPECIES_BAGON] =
            {
                .baseHP        = 45,
        ...
        "#;
        let mut entries = BTreeMap::new();
        parse_species_info_file(text, &mut entries);
        assert_eq!(entries["SPECIES_BAGON"].bst, 300);
        assert!(!entries["SPECIES_BAGON"].fully_evolved);
        assert_eq!(entries["SPECIES_SALAMENCE"].bst, 600);
        assert!(entries["SPECIES_SALAMENCE"].fully_evolved);
    }

    #[test]
    fn lint_detects_mid_rank_power_and_level_drift() {
        let blueprint = test_blueprint("mid");
        let dragonite = test_set("s.dragonite", "SPECIES_DRAGONITE", 50);
        let salamence = test_set("s.salamence", "SPECIES_SALAMENCE", 51);
        let source = test_source("=== TRAINER_X ===\nName: X\n\nSPECIES_BAGON\nLevel: 40\n");
        let cfg = LintConfig::default();
        let species = test_species(&[
            ("SPECIES_DRAGONITE", 600, true),
            ("SPECIES_SALAMENCE", 600, true),
        ]);
        let mut issues = Vec::new();
        run_trainer_lints(
            &blueprint,
        ...
        assert!(issues.iter().any(|i| i.id == "RNK003"));
        assert_eq!(issues.iter().filter(|i| i.id == "RNK004").count(), 2);
    }

    #[test]
    fn lint_notes_late_rank_without_fully_evolved_species() {
        let blueprint = test_blueprint("late");
        let bagon = test_set("s.bagon", "SPECIES_BAGON", 45);
        let shelgon = test_set("s.shelgon", "SPECIES_SHELGON", 45);
        let source = test_source("=== TRAINER_X ===\nName: X\n");
        let cfg = LintConfig::default();
        let species = test_species(&[
            ("SPECIES_BAGON", 300, false),
            ("SPECIES_SHELGON", 420, false),
        ]);
        let mut issues = Vec::new();
        run_trainer_lints(
            &blueprint,
        ...
        assert!(issues
            .iter()
            .any(|i| i.id == "RNK005" && matches!(i.severity, Severity::Note)));
    }
    ...
    fn lint_detects_doubles_mode_mismatch() {
        ...
        let mut issues = Vec::new();
        let species = SpeciesIndex::default();
        run_trainer_lints(&blueprint, &source, &[], &cfg, &species, &mut issues);
