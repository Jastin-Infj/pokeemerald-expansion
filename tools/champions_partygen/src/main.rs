...
fn cmd_generate(opts: &Options) -> Result<i32> {
    ...
    output.push_str("Review with `partygen diff` before enabling B_CHAMPIONS_PARTYGEN_TRAINERS.\n");
    output.push_str("*/\n\n");
...
fn parse_sets(json: &Json) -> Result<Vec<Set>> {
    let root = json.obj()?;
    let default_exp = root.get_string_default("defaultExp", "normal")?;
    validate_exp_policy(default_exp).map_err(|err| format!("defaultExp: {err}"))?;
...
fn validate_exp_policy(value: &str) -> Result<()> {
    match value {
        "normal" | "none" => Ok(()),
        _ => Err(format!(
            "exp must be \"normal\" or \"none\"; got \"{value}\""
        )),
    }
}
...
mod tests {
    ...
    #[test]
    fn rejects_invalid_set_exp_policy() {
        let json = JsonParser::new(
            r#"{
                "schemaVersion": 1,
                "defaultExp": "none",
        ...
        .parse()
        .unwrap();
        let err = match parse_sets(&json) {
            Ok(_) => panic!("invalid exp policy should fail"),
            Err(err) => err,
        };
        assert!(err.contains("s.bad_exp: exp must"));
    }
