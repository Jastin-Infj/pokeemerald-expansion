...
function App() {
  ...
  const selected = useMemo(() => {
    return (
      summary.maps.find((map) => map.directoryName === selectedMapKey) ??
      summary.maps.find((map) => map.name === selectedMapKey) ??
      summary.maps[0] ??
      null
    );
  }, [selectedMapKey, summary.maps]);
  ...
  const scan = useCallback(async (preferredName?: string, rootOverride?: string) => {
    ...
    try {
      ...
      setSelectedMapKey((current) => {
        ...
        if (next.maps.some((map) => map.directoryName === wanted || map.name === wanted)) {
          return wanted;
        }
        return next.maps[0]?.directoryName ?? "";
      });
          ...
          <div className="mapList">
            {filteredMaps.map((map) => (
              <button
                ...
                className={
                  selected?.directoryName === map.directoryName ? "mapRow isSelected" : "mapRow"
                }
                onClick={() => setSelectedMapKey(map.directoryName)}
              >
                <span>{map.name}</span>
              ...
              summary.warnings.map((warning, index) => {
                ...
                return target ? (
                  <button
                    ...
                    onClick={() => setSelectedMapKey(target.directoryName)}
  ...
  const runApply = useCallback(async () => {
    ...
    try {
      const result = await runApplyPlan({
        ...
        renameLayout,
        rewriteScriptLabels,
        planPath: dryRunResult.planPath,
      });
  ...
  return (
    <div className="mapDetail">
      ...
      <div className="relationshipGrid">
        ...
        <Field label="Music" value={map.music || "Unset"} />
        <Field label="Weather" value={map.weather || "Unset"} />
        <Field label="Map Type" value={map.mapType} />
        <Field label="Battle Scene" value={map.battleScene || "Unset"} />
        <Field label="Movement" value={[
          map.allowRunning ? "Run" : "No Run",
          map.allowCycling ? "Bike" : "No Bike",
          map.allowEscaping ? "Escape" : "No Escape",
        ].join(" / ")} />
...
function buildPlanCommand({
  ...
  const script = `${root}/tools/map_asset_relinker/map_relink.py`;
  const outPath = `/tmp/${outName || "map"}_relink.json`;
  const args = [
    "python3",
    script,
    "--root",
