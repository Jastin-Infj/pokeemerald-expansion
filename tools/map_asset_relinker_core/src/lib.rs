...
pub fn make_plan(root: &Path, request: &PlanRequest) -> Result<Value, String> {
    ...
    let layout_plan = if request.rename_layout && request.set_layout_id.is_none() {
...
pub fn apply_plan_dry_run(root: &Path, plan: &Value) -> Result<String, String> {
    ...
    let layout_renames = layout_plans(plan)
        .into_iter()
        .filter_map(|layout| {
            let old_id = plan_string(layout, "oldId")?;
            let new_id = plan_string(layout, "newId")?;
    ...
    for layout in layout_plans(plan) {
        ...
        if let (Some(old_dir), Some(new_dir)) = (old_dir, new_dir) {
            if old_dir != new_dir {
                validate_move_path(root, &old_dir, &new_dir)?;
    ...
    for map_plan in map_plans(plan) {
        ...
        if old_dir != new_dir {
            let old_map_dir = format!("{MAPS_DIR}/{old_dir}");
            let new_map_dir = format!("{MAPS_DIR}/{new_dir}");
            validate_move_path(root, &old_map_dir, &new_map_dir)?;
            output.push(format!(
                "MOVE {}/{} -> {}/{}",
    ...
    for map_plan in map_plans(plan) {
        ...
        let mut map_changed = update_map_json_value(
            &mut map_data,
            &old_ids,
            &new_id,
            &new_name,
        ...
        map_changed |= update_map_refs_value(&mut map_data, &old_ids, &new_id);
        for (old_mapsec, new_mapsec) in &mapsec_renames {
            map_changed |= update_mapsec_refs_value(&mut map_data, old_mapsec, new_mapsec);
        }
        if map_changed {
            print_edit(
                &mut output,
        ...
        if let Some(flag) = plan_string(map_plan, "transitionSetFlag") {
            if let Some(text) = script_text.as_deref() {
                script_changed |= transition_setflag_would_change(
                    text,
                    &new_name,
                    &flag,
                    &[old_dir.clone(), plan_string_required(map_plan, "oldName")?],
                )?;
            }
        }
        if script_changed {
        ...
        for map_json in iter_all_map_json(root)? {
            ...
            refs_changed |= update_layout_refs_value(&mut data, &layout_renames);
            for (old_mapsec, new_mapsec) in &mapsec_renames {
...
pub fn scan_project(root: Option<PathBuf>) -> Result<ProjectSummary, String> {
        ...
        .map_err(|err| format!("failed to read {}: {err}", maps_dir.display()))?
    {
        ...
        if !map_json_path.exists() {
            if is_generated_map_output_dir(&entry.path())? {
                continue;
            }
        ...
        let music = string_field(&map_json, "music").unwrap_or_default();
        let weather = string_field(&map_json, "weather").unwrap_or_default();
        let map_type = string_field(&map_json, "map_type").unwrap_or_default();
        let battle_scene = string_field(&map_json, "battle_scene").unwrap_or_default();
        let allow_running = map_json
            .get("allow_running")
            .and_then(Value::as_bool)
            .unwrap_or(false);
        let allow_cycling = map_json
            .get("allow_cycling")
            .and_then(Value::as_bool)
            .unwrap_or(false);
        let allow_escaping = map_json
            .get("allow_escaping")
            .and_then(Value::as_bool)
            .unwrap_or(false);
        ...
        let groups = map_to_groups
            .get(&directory_name)
            .or_else(|| map_to_groups.get(&name))
            .cloned()
            .unwrap_or_default();
...
fn validate_move_path(root: &Path, src_rel: &str, dst_rel: &str) -> Result<(), String> {
    ...
    if !src.exists() {
        return Err(format!("Cannot move missing path {src_rel}."));
    }
    if dst.exists() {
        return Err(format!("Destination already exists: {dst_rel}."));
    }
    Ok(())
}
...
fn update_map_json_value(
    ...
    let mut changed = false;
    changed |= set_string_field(object, "id", new_id);
...
fn update_layout_refs_value(data: &mut Value, layout_renames: &BTreeMap<String, String>) -> bool {
    let Some(object) = data.as_object_mut() else {
        return false;
    };
    let Some(layout) = object.get("layout").and_then(Value::as_str) else {
        return false;
    };
    let Some(new_layout) = layout_renames.get(layout) else {
        return false;
    };
    object.insert("layout".to_string(), Value::String(new_layout.clone()));
    true
}
...
fn transition_setflag_would_change(
    text: &str,
    map_name: &str,
    flag: &str,
    fallback_prefixes: &[String],
) -> Result<bool, String> {
    let mut prefixes = vec![map_name.to_string()];
    ...
    let mut found = None;
    for prefix in &prefixes {
        let header = format!("{prefix}_MapScripts::");
        if let Some(header_index) = text.find(&header) {
            found = Some((prefix.clone(), header_index));
            break;
        }
    }
    let (script_prefix, header_index) = match found {
        Some(found) => found,
        None => text
            .lines()
            ...
            .find_map(|(offset, line)| {
                line.strip_suffix("_MapScripts::")
                    .map(|prefix| (prefix.to_string(), offset))
            })
            .ok_or_else(|| {
                let expected = prefixes
                    .iter()
                    .map(|prefix| format!("{prefix}_MapScripts::"))
                    .collect::<Vec<_>>()
                    .join(", ");
                format!("Could not find map script header; expected one of: {expected}.")
            })?,
    };
    if text[header_index..].find("\t.byte 0").is_none() {
        return Err(format!(
            "Could not find map script terminator for {script_prefix}."
        ));
    }
    let byte_index = header_index
        + text[header_index..]
            .find("\t.byte 0")
            .ok_or_else(|| format!("Could not find map script terminator for {script_prefix}."))?;
    let script_table = &text[header_index..byte_index];
    let transition_label = script_table.lines().find_map(|line| {
        let trimmed = line.trim();
        trimmed
            .strip_prefix("map_script")
            .and_then(|rest| rest.trim().strip_prefix("MAP_SCRIPT_ON_TRANSITION,"))
            .map(str::trim)
            .filter(|label| !label.is_empty())
            .map(str::to_string)
    });
    if let Some(label) = transition_label {
        if let Some(label_start) = find_script_label_body_start(text, &label) {
            let label_end = text[label_start..]
                .lines()
                .scan(label_start, |offset, line| {
                    let current = *offset;
                    *offset += line.len() + 1;
                    Some((current, line))
                })
                .skip(1)
                .find_map(|(offset, line)| is_script_label_line(line).then_some(offset))
                .unwrap_or(text.len());
            if text[label_start..label_end].contains(&format!("setflag {flag}")) {
                return Ok(false);
            }
        }
    }
...
fn find_script_label_body_start(text: &str, label: &str) -> Option<usize> {
    [format!("{label}::\n"), format!("{label}:\n")]
        .into_iter()
        .find_map(|header| text.find(&header).map(|index| index + header.len()))
}

fn is_script_label_line(line: &str) -> bool {
    line.strip_suffix("::")
        .or_else(|| line.strip_suffix(':'))
        .is_some_and(|label| !label.is_empty() && label.chars().all(is_identifier_char))
}
...
fn flag_claim_would_change(text: &str, claim: &Value) -> Result<bool, String> {
    ...
    let new_exists = text.lines().any(|line| line.starts_with(&new_define));
    if text.lines().any(|line| line.starts_with(&old_define)) {
        if new_exists && old_flag != new_flag {
            return Err(format!(
                "Cannot claim {old_flag} as {new_flag}; {new_flag} already exists."
            ));
        }
        return Ok(true);
    }
...
fn is_generated_map_output_dir(path: &Path) -> Result<bool, String> {
        ...
        fs::read_dir(path).map_err(|err| format!("failed to read {}: {err}", path.display()))?
    {
        ...
        if matches!(name, "header.inc" | "events.inc" | "connections.inc") {
            saw_generated_output = true;
        } else {
            return Ok(false);
        }
    }
    Ok(saw_generated_output)
}
