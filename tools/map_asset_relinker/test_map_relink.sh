...
assert_command_fails() {
	expected=$1
	shift
	log=$(mktemp /tmp/maprelink-command-fail.XXXXXX)
	if "$@" >"$log" 2>&1; then
...
empty_map_id_tmpdir=$(mktemp -d /tmp/maprelink-empty-map-id-test.XXXXXX)
cp -R "$fixture/." "$empty_map_id_tmpdir/"
python3 -c 'import json, sys; p = sys.argv[1]; data = json.load(open(p)); data["id"] = ""; json.dump(data, open(p, "w"), indent=2); open(p, "a").write("\n")' "$empty_map_id_tmpdir/data/maps/OldCave_2/map.json"
assert_audit_fails "$empty_map_id_tmpdir" "must have an id string starting with MAP_"
python3 "$script_dir/map_relink.py" --root "$empty_map_id_tmpdir" plan \
	--map OldCave_2:OldCave_2F \
	--from-group gMapGroup_Temp \
	--to-group gMapGroup_RougeCave \
	--out "$empty_map_id_tmpdir/repair.json"
python3 "$script_dir/map_relink.py" --root "$empty_map_id_tmpdir" apply --allow-dirty "$empty_map_id_tmpdir/repair.json" >/dev/null
python3 "$script_dir/map_relink.py" --root "$empty_map_id_tmpdir" validate
python3 -c 'import json, sys; data = json.load(open(sys.argv[1])); assert data["id"] == "MAP_OLD_CAVE_2F"' "$empty_map_id_tmpdir/data/maps/OldCave_2F/map.json"
...
set_layout_id_default_tmpdir=$(mktemp -d /tmp/maprelink-set-layout-id-default-test.XXXXXX)
cp -R "$fixture/." "$set_layout_id_default_tmpdir/"
python3 "$script_dir/map_relink.py" --root "$set_layout_id_default_tmpdir" plan \
	--map OldCave_2:OldCave_2F \
	--set-layout-id LAYOUT_OLD_CAVE_EXIT \
	--out "$set_layout_id_default_tmpdir/repair.json"
python3 -c 'import json, sys; data = json.load(open(sys.argv[1])); assert data["layouts"] == []' "$set_layout_id_default_tmpdir/repair.json"
python3 "$script_dir/map_relink.py" --root "$set_layout_id_default_tmpdir" apply --allow-dirty "$set_layout_id_default_tmpdir/repair.json" >/dev/null
python3 "$script_dir/map_relink.py" --root "$set_layout_id_default_tmpdir" validate
test -d "$set_layout_id_default_tmpdir/data/layouts/OldCave_2"
test ! -e "$set_layout_id_default_tmpdir/data/layouts/OldCave_2F"
python3 -c 'import json, sys; data = json.load(open(sys.argv[1])); assert data["layout"] == "LAYOUT_OLD_CAVE_EXIT"' "$set_layout_id_default_tmpdir/data/maps/OldCave_2F/map.json"

shared_layout_tmpdir=$(mktemp -d /tmp/maprelink-shared-layout-test.XXXXXX)
cp -R "$fixture/." "$shared_layout_tmpdir/"
cp -R "$shared_layout_tmpdir/data/maps/OldCave_Exit" "$shared_layout_tmpdir/data/maps/OtherShared"
python3 -c 'import json, sys; p = sys.argv[1]; data = json.load(open(p)); data["id"] = "MAP_OTHER_SHARED"; data["name"] = "OtherShared"; data["layout"] = "LAYOUT_OLD_CAVE_2"; json.dump(data, open(p, "w"), indent=2); open(p, "a").write("\n")' "$shared_layout_tmpdir/data/maps/OtherShared/map.json"
python3 -c 'import json, sys; p = sys.argv[1]; data = json.load(open(p)); data["gMapGroup_Temp"].append("OtherShared"); json.dump(data, open(p, "w"), indent=2); open(p, "a").write("\n")' "$shared_layout_tmpdir/data/maps/map_groups.json"
printf '\t.include "data/maps/OtherShared/scripts.inc"\n' >> "$shared_layout_tmpdir/data/event_scripts.s"
python3 "$script_dir/map_relink.py" --root "$shared_layout_tmpdir" plan \
	--map OldCave_2:OldCave_2F \
	--out "$shared_layout_tmpdir/repair.json"
python3 "$script_dir/map_relink.py" --root "$shared_layout_tmpdir" apply --dry-run "$shared_layout_tmpdir/repair.json" | grep -q 'EDIT data/maps/OtherShared/map.json'
python3 "$script_dir/map_relink.py" --root "$shared_layout_tmpdir" apply --allow-dirty "$shared_layout_tmpdir/repair.json" >/dev/null
python3 "$script_dir/map_relink.py" --root "$shared_layout_tmpdir" validate
python3 -c 'import json, sys; data = json.load(open(sys.argv[1])); assert data["layout"] == "LAYOUT_OLD_CAVE_2F"' "$shared_layout_tmpdir/data/maps/OtherShared/map.json"
...
duplicate_flag_tmpdir=$(mktemp -d /tmp/maprelink-duplicate-flag-test.XXXXXX)
cp -R "$fixture/." "$duplicate_flag_tmpdir/"
mkdir -p "$duplicate_flag_tmpdir/include/constants"
cat > "$duplicate_flag_tmpdir/include/constants/flags.h" <<'EOF'
#define FLAG_VISITED_LITTLEROOT_TOWN                (SYSTEM_FLAGS + 0xF)
#define FLAG_UNUSED_0x881                           (SYSTEM_FLAGS + 0x21) // Unused Flag
EOF
python3 "$script_dir/map_relink.py" --root "$duplicate_flag_tmpdir" plan \
	--map OldCave_2:OldCave_2 \
	--no-layout-rename \
	--claim-unused-flag FLAG_UNUSED_0x881:FLAG_VISITED_LITTLEROOT_TOWN \
	--out "$duplicate_flag_tmpdir/duplicate_flag.json"
assert_command_fails "already exists" python3 "$script_dir/map_relink.py" --root "$duplicate_flag_tmpdir" apply --allow-dirty "$duplicate_flag_tmpdir/duplicate_flag.json"

numeric_flag_tmpdir=$(mktemp -d /tmp/maprelink-numeric-flag-test.XXXXXX)
cp -R "$fixture/." "$numeric_flag_tmpdir/"
mkdir -p "$numeric_flag_tmpdir/include/constants"
cat > "$numeric_flag_tmpdir/include/constants/flags.h" <<'EOF'
#define FLAG_UNUSED_0x020    0x20 // Unused Flag
EOF
python3 "$script_dir/map_relink.py" --root "$numeric_flag_tmpdir" plan \
	--map OldCave_2:OldCave_2 \
	--no-layout-rename \
	--claim-unused-flag FLAG_UNUSED_0x020:FLAG_VISITED_OLD_CAVE \
	--out "$numeric_flag_tmpdir/numeric_flag.json"
python3 "$script_dir/map_relink.py" --root "$numeric_flag_tmpdir" apply --allow-dirty "$numeric_flag_tmpdir/numeric_flag.json" >/dev/null
grep -q '#define FLAG_VISITED_OLD_CAVE' "$numeric_flag_tmpdir/include/constants/flags.h"
grep -q '0x20' "$numeric_flag_tmpdir/include/constants/flags.h"

move_collision_tmpdir=$(mktemp -d /tmp/maprelink-move-collision-test.XXXXXX)
cp -R "$fixture/." "$move_collision_tmpdir/"
cp -R "$move_collision_tmpdir/data/maps/OldCave_Exit" "$move_collision_tmpdir/data/maps/OldCave_2F"
python3 "$script_dir/map_relink.py" --root "$move_collision_tmpdir" plan \
	--map OldCave_2:OldCave_2F \
	--out "$move_collision_tmpdir/collision.json"
assert_command_fails "Destination already exists" python3 "$script_dir/map_relink.py" --root "$move_collision_tmpdir" apply --dry-run "$move_collision_tmpdir/collision.json"
assert_command_fails "Destination already exists" python3 "$script_dir/map_relink.py" --root "$move_collision_tmpdir" apply --allow-dirty "$move_collision_tmpdir/collision.json"
test -d "$move_collision_tmpdir/data/layouts/OldCave_2"
test ! -e "$move_collision_tmpdir/data/layouts/OldCave_2F"
...
python3 "$script_dir/map_relink.py" --root "$broken_warp_tmpdir" apply --dry-run "$broken_warp_tmpdir/repair.json" | grep -q 'EDIT data/maps/OldCave_2/map.json'
...
python3 -c 'import json, sys; data = json.load(open(sys.argv[1])); assert data["warp_events"][0]["dest_map"] == "MAP_OLD_CAVE_2"' "$broken_warp_tmpdir/data/maps/OldCave_2/map.json"
...
transition_no_rewrite_tmpdir=$(mktemp -d /tmp/maprelink-transition-no-rewrite-test.XXXXXX)
cp -R "$fixture/." "$transition_no_rewrite_tmpdir/"
cat >> "$transition_no_rewrite_tmpdir/data/maps/OldCave_2/scripts.inc" <<'EOF'

OldCave_2_EventScript_AlreadySetsFlag::
	setflag FLAG_VISITED_OLD_CAVE
	end
EOF
python3 "$script_dir/map_relink.py" --root "$transition_no_rewrite_tmpdir" plan \
	--map OldCave_2:OldCave_2F \
	--from-group gMapGroup_Temp \
	--to-group gMapGroup_RougeCave \
	--set-transition-setflag FLAG_VISITED_OLD_CAVE \
	--out "$transition_no_rewrite_tmpdir/repair.json"
python3 "$script_dir/map_relink.py" --root "$transition_no_rewrite_tmpdir" apply --dry-run "$transition_no_rewrite_tmpdir/repair.json" | grep -q 'Dry-run complete; no files changed.'
python3 "$script_dir/map_relink.py" --root "$transition_no_rewrite_tmpdir" apply --allow-dirty "$transition_no_rewrite_tmpdir/repair.json" >/dev/null
python3 "$script_dir/map_relink.py" --root "$transition_no_rewrite_tmpdir" validate
grep -q 'OldCave_2_MapScripts::' "$transition_no_rewrite_tmpdir/data/maps/OldCave_2F/scripts.inc"
grep -q 'map_script MAP_SCRIPT_ON_TRANSITION, OldCave_2_OnTransition' "$transition_no_rewrite_tmpdir/data/maps/OldCave_2F/scripts.inc"
grep -q 'setflag FLAG_VISITED_OLD_CAVE' "$transition_no_rewrite_tmpdir/data/maps/OldCave_2F/scripts.inc"

transition_double_colon_tmpdir=$(mktemp -d /tmp/maprelink-transition-double-colon-test.XXXXXX)
cp -R "$fixture/." "$transition_double_colon_tmpdir/"
cat > "$transition_double_colon_tmpdir/data/maps/OldCave_2/scripts.inc" <<'EOF'
OldCave_2_MapScripts::
	map_script MAP_SCRIPT_ON_TRANSITION, OldCave_2_OnTransition
	.byte 0

OldCave_2_OnTransition::
	end
EOF
python3 "$script_dir/map_relink.py" --root "$transition_double_colon_tmpdir" plan \
	--map OldCave_2:OldCave_2 \
	--no-layout-rename \
	--set-transition-setflag FLAG_VISITED_OLD_CAVE \
	--out "$transition_double_colon_tmpdir/repair.json"
python3 "$script_dir/map_relink.py" --root "$transition_double_colon_tmpdir" apply --allow-dirty "$transition_double_colon_tmpdir/repair.json" >/dev/null
python3 "$script_dir/map_relink.py" --root "$transition_double_colon_tmpdir" validate
python3 -c 'import sys; text = open(sys.argv[1]).read(); assert text.count("OldCave_2_OnTransition") == 2; assert "OldCave_2_OnTransition::\n\tsetflag FLAG_VISITED_OLD_CAVE\n\tend" in text' "$transition_double_colon_tmpdir/data/maps/OldCave_2/scripts.inc"

echo "map_asset_relinker fixture test passed: $tmpdir $missing_group_tmpdir $broken_group_tmpdir $broken_map_name_tmpdir $broken_map_id_tmpdir $empty_map_id_tmpdir $broken_layout_tmpdir $foreign_layout_tmpdir $set_layout_id_default_tmpdir $shared_layout_tmpdir $broken_mapsec_tmpdir $map_name_anchor_tmpdir $temp_mapsec_tmpdir $tileset_fix_tmpdir $layout_name_fix_tmpdir $duplicate_mapsec_tmpdir $mapsec_name_fix_tmpdir $region_map_tools_tmpdir $duplicate_flag_tmpdir $numeric_flag_tmpdir $move_collision_tmpdir $broken_warp_tmpdir $match_by_name_tmpdir $match_by_id_tmpdir $duplicate_script_tmpdir $script_label_tmpdir $transition_no_rewrite_tmpdir $transition_double_colon_tmpdir"
